...
name: Ureteral Calculi — Imaging Sensitivity
subspecialty: [ABD, US]
aliases:
  - urolithiasis CT
  - ureteric calculi detection
  - 輸尿管結石影像
dateRev: 2026-06-17
---
...
- **超音波**：常為泌尿道首檢,但對結石**敏感度低**（一研究以 CT KUB 為參考,US 敏感度僅 ~24%,漏掉者近 75% <3mm);徵象＝回音灶、acoustic shadowing、twinkle artifact。[^1]
- **IVU（靜脈尿路攝影）**：已**主要被非顯影 CT 取代**。[^1]
- **KUB 平片**：許多結石透 X 光、與骨/糞便重疊,敏感度低。[^1]

## 放射科醫師影像判讀重點
- **NCCT 為金標準（Vieweg，數據）**：**非顯影螺旋 CT 診斷輸尿管結石之敏感度 98%、特異度 98%、準確度 96%**——已成急性脅腹痛評估標準，多數病人無需進一步影像即可決定是否泌尿科介入。[^3]
- **CT 直接與間接徵**：**結石本體高衰減（直接）＋腎盂輸尿管積水、腎周/輸尿管周脂肪條紋、rim/tissue-rim sign（間接）**；間接徵有助辨識已排出或微小結石。[^1][^3]
...
2. **臨床路徑（Vieweg）**：**NCCT 使急診脅腹痛快速分流**，可同時排除其他腹內急症（57% CT 陰性者由其他所見得診斷）。[^3]
3. **輻射考量**：CT 輻射為缺點，孕婦/兒童/反覆追蹤者優先 US 或低劑量 CT。[^1]
4. **處置分流**：小且遠端結石多保守排石（±α-blocker）；大、近端、阻塞併感染/腎功能惡化者需輸尿管鏡/ESWL/引流。[^2][^3]
5. **成分線索**：CT 衰減值可提示結石成分（尿酸低、草酸鈣/磷酸鈣高、cystine 中等），影響治療選擇。[^1]

> [!note] 考點：偵測輸尿管結石敏感度最高的檢查?
> **非顯影螺旋 CT（unenhanced helical CT）**。US 敏感度低(~24%)、KUB 漏透 X 光結石、IVU 已被 CT 取代。[^1]

> [!note] 自發排出率主要取決於『結石大小』
> 自發排出率（20 週內、軸向徑）：**0–3 mm：98%；4 mm：81%；5 mm：65%；6 mm：33%；>6.5 mm：9%**。**結石越小排出率越高**;故 **UVJ 的 3 mm 結石（98%）排出率最高**,大於近端 9 mm／中段 7 mm／遠端 5 mm。位置（遠端較易過 UVJ）為次要因子。[^2]

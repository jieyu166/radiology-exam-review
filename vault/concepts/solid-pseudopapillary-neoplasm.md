---
concepts: [solid-pseudopapillary-neoplasm]
name: Solid Pseudopapillary Neoplasm of Pancreas (SPN)
subspecialty: [ABD]
aliases:
  - solid pseudopapillary tumor
  - SPN
  - SPEN
  - Hamoudi tumor
  - 實性偽乳頭狀腫瘤
dateRev: 2026-06-18
---

# solid-pseudopapillary-neoplasm

**SPN＝年輕女性的少見、低惡性度胰臟腫瘤,典型為大而有出血/囊變的混合性腫塊。** 判讀分水嶺一：**好發年輕女性（20–30 歲）**;判讀分水嶺二：**大（常 >5 cm）、有出血/囊變、纖維包膜、可鈣化;低惡性潛能、預後佳**。

## Summary
- **流行病學**：**少見（占外分泌胰臟腫瘤 1–2%）**;**典型年輕女性**。[^1]
- **影像**：**大（中位 ~8 cm）、邊界清、有包膜、實質＋出血/囊變混合、可周邊鈣化**;太小時可全實心。[^1]
- **生物行為**：**低惡性潛能、轉移少見、預後佳**（非高度惡性易轉移）。[^1]

> [!note] 考點：關於胰臟 SPN,何者「正確」?
> 「**好發年輕女性**」正確。其為**少見**腫瘤（非常見,A 誤）、因常**大且有囊變**而有症狀（非因小而無症狀,C 誤）、**低惡性潛能**（非高度惡性易轉移,D 誤）。[^1]

### 參考來源
[^1]: *Solid pseudopapillary tumour of the pancreas*. Radiopaedia.org（實際查證 accessed 2026-06-18 ／題目所引）：少見（1–2% 外分泌胰腫瘤）、**年輕女性**;大而有包膜、實質＋出血/囊變、可鈣化;低惡性潛能、預後佳。

## 題目
> [!question]- 關於胰臟 solid and pseudopapillary tumor,何者「正確」? (2017-021)
> **好發年輕女性**。SPN 為少見腫瘤、常大而有囊變故有症狀、低惡性潛能（轉移少見）;其餘三項皆誤。[^1]

## 考題
```dataview
list from #交換 where contains(concepts, "solid-pseudopapillary-neoplasm")
```

...
## 技術要點
- **頻寬與 SNR（核心考點）**：SNR ∝ **1/√BW**；接收頻寬減半 → SNR 升為 **√2 倍（≈1.41×）**，不是 2 倍——這是最常誤判為「線性」的陷阱。[^1]
- **窄頻寬的代價**：提高 SNR 的同時會**延長最短 TE、加重化學位移偽影**，故不能一味縮窄頻寬；臨床須與對比、偽影權衡。[^1]
- **√ 依存的參數群**：SNR ∝ **√(NEX)** 與 **√(相位編碼步數)**——訊號平均或相位步數加倍僅換得 √2 倍 SNR，屬「時間換 SNR」的低效率手段。[^1][^3]
- **體素體積是線性旋鈕**：SNR ∝ **voxel volume**；放大體素（增層厚/FOV 或降矩陣）以線性方式大幅提升 SNR，但犧牲空間解析度——比堆疊 NEX 更有效率。[^3]
- **場強與 SNR**：完美系統下 SNR 與 **B0 成正比**（訊號 ∝ B0²、雜訊 ∝ B0），故 3T 理論 SNR 為 1.5T 之 **2 倍**；線圈設計、載入雜訊等使實務略低於理論值。[^2]
- **成像鏈觀點（Plewes）**：SNR 根本取決於**體素內自旋數（voxel 大小/質子密度）、B₀、線圈與取樣（頻寬/平均次數）**；3D volume 取像因激發整塊組織，較 2D 增 SNR。[^3][^4]
- **判讀提醒**：SNR 不足時影像顆粒感重、低對比病灶易被雜訊淹沒；調參時「頻寬、NEX、體素、場強、線圈」須整體考量，非單一旋鈕。[^4]

---
concepts: [adrenal-lesions]
name: Adrenal Lesions (Hyperplasia, Pseudocyst, Pheochromocytoma)
subspecialty: [ABD]
aliases:
  - adrenal nodular hyperplasia
  - adrenal pseudocyst
  - pheochromocytoma calcification
  - 腎上腺病灶
dateRev: 2026-06-29
---

# adrenal-lesions

**腎上腺三題核心：增生「隨齡雙側」、pseudocyst「T1 亮因出血」、pheo「10% 鈣化」。** 結節性增生隨年齡盛行率上升、多雙側瀰漫（非局灶單側）；pseudocyst T1 高訊號來自出血（非脂肪/鈣化/單純液）；pheochromocytoma 的「10% rule」中含鈣化約 10%。這三點直接對應三道獨立考題。

## Summary
- **Adrenal cortical nodular hyperplasia**：**隨年齡盛行率增加**;多**雙側/瀰漫**（非局灶、非單側）;可與腎上腺皮質腺瘤並存。[^1]
- **Adrenal pseudocyst**：T1WI 內**高訊號最可能源自出血(hemorrhage)**（假性囊腫常為出血後);非脂肪/鈣化/單純液。[^1]
- **Pheochromocytoma 鈣化**：約 **10%** 含鈣化。[^1]

> [!note] 考點
> 結節性增生**隨年齡增加、雙側**;adrenal pseudocyst T1 高訊號＝**出血**;pheochromocytoma 約 **10%** 鈣化。[^1]

## 影像判讀骨架（一般放射科醫師需知）

### 第一線：非顯影 CT 衰減值
- **≤10 HU（多測 ROI、避開鈣化/壞死）即可確診 lipid-rich adenoma**，無論大小、不需追加檢查;此閾值對 lipid-rich adenoma 敏感度約 71%、特異度約 98%（含脂量足以拉低衰減值）。[^2][^3]
- **11–30 HU 為 lipid-poor adenoma 範圍，HU 單獨無法判讀**;需追加 CT washout 或 chemical-shift MRI 才能確認。[^2][^3]
- 衰減值同時是鑑別第一步：**含巨觀脂肪（負值，約 −20 至 −120 HU）→ myelolipoma**;高衰減/不均/壞死/出血則指向 pheo、ACC、轉移。[^4][^5]

### CT washout（adrenal protocol：非顯影 + 門脈期約 60–70 秒 + 15 min 延遲）
- **APW（絕對洗出率）= [(門脈期 HU − 延遲 HU) / (門脈期 HU − 非顯影 HU)] × 100%;≥60% 提示腺瘤。** 敏感度約 86–88%、特異度約 92–96%。[^1][^6]
- **RPW（相對洗出率）= [(門脈期 HU − 延遲 HU) / 門脈期 HU] × 100%;≥40% 提示腺瘤。** 用於無非顯影影像時;敏感度約 82–83%、特異度約 92–93%。[^1][^6]
- 部分文獻採 10 min 延遲協定亦有效;延遲時間（10 vs 15 min）來源略有分歧，計算公式相同。[^6]

### Chemical-shift MRI（in-phase / opposed-phase）
- 腺瘤含**胞內脂肪 → opposed-phase（反相位）相對 in-phase 訊號掉落**（同一 voxel 內水＋脂相消）。視覺以脾為內參。[^2][^7]
- **SII（signal intensity index）= [(IP − OOP) / IP] × 100%;1.5T >16.5% 提示 lipid-rich adenoma。** [^7]
- **Adrenal-spleen ratio（ASR）= (腺瘤OOP/脾OOP) / (腺瘤IP/脾IP);<0.71 提示腺瘤。** SII 與 ASR 為最可靠的兩項定量指標。[^7]

### 各病灶影像鑑別
- **Adenoma**：小、邊界清、均質;非顯影 ≤10 HU 或 CSI 訊號掉落即診斷;lipid-poor 者靠 washout。[^2][^3]
- **Pheochromocytoma**：**T2 顯著高訊號（"light bulb sign"，惟僅約一部分典型，約 35% 反呈低/中等訊號）**、富血管明顯強化、可囊變/出血/壞死;washout 可與腺瘤重疊（見陷阱）。[^4][^8]
- **Myelolipoma**：**含巨觀脂肪**（CT 負值區、MRI T1/T2 高訊號且脂肪抑制掉訊號）為診斷依據。[^4][^5]
- **Metastasis**：常雙側、邊界較不規則、無胞內脂肪（CSI 不掉訊號）、washout 偏低;T2 多中等偏高。[^2][^8]
- **ACC（腎上腺皮質癌）**：通常**大（多 >4–6 cm）且不均質**，含壞死/出血/鈣化，強化不均、可侵犯血管;少數含巨觀脂肪。[^4]
- **出血（hemorrhage）**：可雙側、無強化、隨時間訊號演變（T1 隨亞急性期高訊號）;對應假性囊腫前驅。[^1]
- **囊腫（cyst）**：單純液體衰減/訊號、薄壁、無強化;假性囊腫多為出血後，T1 高訊號源自出血而非脂肪。[^1]

### 判讀陷阱
- **lipid-poor adenoma（11–30 HU）不能只看 HU**，須 washout 或 CSI;反之 CSI 無訊號掉落不等於排除腺瘤（脂量不足）。[^2][^3]
- **pheochromocytoma 的 washout 可落在腺瘤範圍**，washout 高不能排除 pheo;有臨床/生化或顯著 T2 高訊號時須警覺。[^4][^8]
- **≤10 HU 但合併鈣化或壞死區**：ROI 須避開鈣化/壞死，否則衰減值失真;鈣化本身非腺瘤特徵（pheo 約 10% 含鈣化）。[^1][^4]
- washout 對**較大、含壞死/出血的病灶**準確度下降;ACC 與 pheo 偶可呈高 washout，washout 僅為「中等」鑑別工具。[^6]

### 參考來源
[^1]: **Tier 1（全文已讀，via 個人訂閱 pubs.rsna.org，accessed 2026-07-01；同源見 [[adrenal-imaging]][^2]）** Lattin GE, Sturgill ED, Tujo CA, et al. *From the Radiologic Pathology Archives: Adrenal Tumors and Tumor-like Conditions in the Adult*. RadioGraphics. 2014;34(3):805-829. https://doi.org/10.1148/rg.343130127。腎上腺結節性增生隨年齡增加/雙側；假性囊腫 T1 高訊號多因出血；嗜鉻細胞瘤約 10% 鈣化。
[^2]: ACR Incidental Findings Committee White Paper（管理指引層級），Mayo-Smith WW et al., J Am Coll Radiol 2017;14(8):1038-1044：非顯影 ≤10 HU 或 CSI 對脾訊號掉落幾乎確診 lipid-rich adenoma（不論大小）;11–30 HU 之 lipid-poor adenoma 須 washout 或 CSI。https://doi.org/10.1016/j.jacr.2017.05.001
[^3]: AJR 評論層級，Corwin MT et al.（Adrenal Incidentalomas: Clinical Controversies and Modified Recommendations），AJR 2016：≤10 HU 對 lipid-rich adenoma 敏感度約 71%、特異度約 98%;lipid-poor adenoma 11–30 HU 不能僅憑 HU 判讀。https://doi.org/10.2214/AJR.15.15475
[^4]: RadioGraphics 教學文章層級，Blake MA et al.（Adrenal Mass Imaging with Multidetector CT: Pathologic Conditions, Pearls, and Pitfalls），RadioGraphics 2009;29(5):1333-1351：各病灶影像特徵與 washout/CSI 陷阱（pheo T2 高訊號/明顯強化、myelolipoma 含脂肪、ACC 大且不均、washout 重疊）。https://doi.org/10.1148/rg.295095027
[^5]: SAR 疾病聚焦小組命名共識（期刊論文層級），Schieda N et al.，J Magn Reson Imaging 2019;50(1):51-64：腎上腺含脂病灶（巨觀脂肪 vs 胞內脂肪）影像命名;myelolipoma 以巨觀脂肪為診斷依據。https://doi.org/10.1002/jmri.26542
[^6]: Radiology 原著研究層級，Caoili EM 等延遲洗出協定相關文獻（10–15 min delayed washout protocol）：APW ≥60%、RPW ≥40% 公式與診斷效能（敏感度/特異度如內文）;washout 為中等診斷價值工具。對應 PMC 開放全文（Schloetelburg et al., Eur J Endocrinol 2022;186(2):183-193）所述「moderate diagnostic value」。https://pmc.ncbi.nlm.nih.gov/articles/PMC8679842/
[^7]: AJR 定量比較研究層級，Israel GM et al. 與 1.5T/3T CSI 量化文獻：SII = [(IP−OOP)/IP]×100%，1.5T >16.5% 提示 lipid-rich adenoma;ASR = (腺瘤OOP/脾OOP)/(腺瘤IP/脾IP)，<0.71 提示腺瘤;兩者為最可靠定量指標。https://doi.org/10.2214/AJR.14.12941
[^8]: Radiology: Imaging Cancer 原著層級（T2 鑑別 lipid-poor adenoma 與轉移）2020;2(6):e200011，及 PMC 開放全文「MRI of adrenal gland: state of the art」（Adam SZ et al., PMC6755942）：pheo 顯著 T2 高訊號（light bulb sign，約 35% 不典型）;轉移無胞內脂肪、CSI 不掉訊號。https://pmc.ncbi.nlm.nih.gov/articles/PMC6755942/

## 考題
```dataview
list from #交換 where contains(concepts, "adrenal-lesions")
```

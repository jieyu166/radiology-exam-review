---
concepts: [capillary-telangiectasia]
name: CNS Capillary Telangiectasia
subspecialty: [NR]
aliases:
  - capillary telangiectasia
  - brain capillary telangiectasia
  - 微血管擴張
dateRev: 2026-06-17
same:
  - "[[cerebral-cavernous-malformation]]"
---

# capillary-telangiectasia

**腦微血管擴張是一簇擴張的微血管，血管之間「夾有正常腦實質」——這正是與 cavernoma 最核心的區別（cavernoma 血管之間無腦實質）。** 好發橋腦（約 80%），多無症狀，對比後呈點狀/花邊狀淡強化，SWI 上有 blooming 低訊號。記憶鉤：「毛細血管海綿夾著正常腦、cavernoma 是純血管沒有腦」——有沒有夾腦組織，決定診斷。

## Summary
- **好發橋腦（~80%）**(option A 對);亦見中腦、基底節、皮質。[^1]
- **病理**：擴張微血管,**間夾正常腦實質（interspersed with normal brain parenchyma）**;故「dilated capillaries **without** intervening brain parenchyma」**錯誤**＝題目正解（2016-186 正解 B）——「無間質腦組織」才是 cavernoma 的特徵。[^1]
- **多無症狀、偶見**(option C 對)。[^1]
- **影像**：T1/T2 多不顯;對比後**花邊狀/點狀（lacelike/stippled）淡強化**(option D 對);**SWI/GRE 低訊號（blooming）**。[^1]

> [!note] 考點：capillary telangiectasia 何者「錯」?
> 「dilated capillaries **without** intervening brain parenchyma」錯——其**間夾正常腦實質**（無間質腦組織者是 cavernoma）。好發橋腦、無症狀、點狀淡強化皆對。[^1]

### 參考來源
[^1]: *CNS capillary telangiectasia*. Radiopaedia.org（實際查證 accessed 2026-06-17）：好發 **pons（80%）**;擴張微血管「**interspersed with normal brain parenchyma**」;多無症狀偶見;對比後 stippled enhancement、SWI 低訊號。

## 題目
> [!question]- 關於 capillary telangiectasia,何者「錯」?
> 「dilated capillaries **without** intervening brain parenchyma」錯——其**間夾正常腦實質**（cavernoma 才無）。好發橋腦、無症狀、點狀淡強化皆對。[^1]

## 考題
```dataview
list from #交換 where contains(concepts, "capillary-telangiectasia")
```

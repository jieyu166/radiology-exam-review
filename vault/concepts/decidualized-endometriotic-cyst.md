---
concepts: [decidualized-endometriotic-cyst]
name: Decidualized Endometriotic Cyst — MRI vs Ovarian Carcinoma
subspecialty: [MRI, US]
aliases:
  - decidualized endometriosis
  - decidualized endometriotic cyst
  - pregnancy endometrioma mural nodule
  - 蛻膜化子宮內膜異位囊腫
dateRev: 2026-07-03
same:
...
**懷孕期荷爾蒙變化使異位子宮內膜之基質細胞增生（蛻膜化，decidualization），在 endometrioma 內形成實質壁結節，可被誤認為惡性——關鍵鑑別是『ADC 定量值』：蛻膜化組織 ADC 顯著高於卵巢癌，但兩者的 DWI 高訊號本身皆非特異（無法單靠 DWI 訊號區分）。** 判讀分水嶺：**DWI 高訊號對蛻膜化與惡性壁結節皆非特異；ADC 定量蛻膜化顯著高於惡性（蛻膜化基質富細胞質 + 水腫血管化）；T2 蛻膜化壁結節略高於惡性；蛻膜化通常於生產或終止妊娠後消退。**

## Summary
- **機轉/背景**：懷孕期荷爾蒙變化 → 異位子宮內膜基質**蛻膜化**，可在 endometrioma 內形成**實質（可血管化）壁結節**，模擬惡性。[^1][^2]
- **蛻膜化組件定義**：囊壁或囊內隔上**緩慢生長之成分**均視為蛻膜化組件。[^1]
- **T2**：蛻膜化實質成分 **T2 訊號略高於**惡性（卵巢癌）壁結節。[^1]
- **DWI/ADC（關鍵）**：**DWI 高訊號對兩者皆非特異**、無法單靠訊號區分；但**ADC 定量值蛻膜化顯著高於卵巢癌**（蛻膜化基質富細胞質 + 水腫血管化）。[^1]
...
> ⚠️ 官方答案為 D，但依所附段落，A（蛻膜化組件定義）、B（T2 蛻膜化略高於惡性）、C（ADC 蛻膜化顯著高於惡性）皆為文獻支持之正確敘述。D 為研究背景性描述，其被判定為偽的確切理由建議醫師對照原文全段確認。DWI 高訊號本身對兩者皆非特異，需靠 ADC 定量值鑑別。[^1]

## 考題
```dataview
list from #交換 where contains(concepts, "decidualized-endometriotic-cyst")
```

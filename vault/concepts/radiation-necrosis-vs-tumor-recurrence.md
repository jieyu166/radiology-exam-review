---
concepts: [radiation-necrosis-vs-tumor-recurrence]
name: Radiation Necrosis vs Brain Tumor Recurrence
subspecialty: [NR]
aliases:
  - lesion quotient
  - cut green pepper sign
  - soap bubble sign radiation necrosis
  - 放射線壞死
dateRev: 2026-07-01
---
...
**放射線壞死(radiation necrosis)與腦腫瘤復發在傳統MRI上常難以區分，但「lesion quotient（病灶商數）」是一個量化輔助指標——病灶商數=T2WI上低訊號結節面積÷對比後T1WI上同病灶面積；腫瘤復發病灶商數較「高」（>0.6），放射線壞死則較「低」（<0.3）。** 判讀分水嶺：**放射線壞死可依原發腫瘤位置與放射照野(radiation port)推測好發部位;腦室旁白質因長髓質動脈供血差、缺乏側枝循環，對放射線引起的血管病變特別脆弱，故照野內腦室旁強化灶多屬放射線引起，可考慮追蹤觀察其消退/穩定；"cut green pepper"或"soap bubble"樣強化外觀雖可提示放射線壞死，但研究(Dequesada等)顯示此外觀並不能可靠區分放射線壞死與腫瘤（兩者皆可出現，也常見於混合病灶）。**

## Summary
- **好發部位推測**：放射線壞死可依**原發腫瘤位置與放射照野(radiation port)**範圍加以推測好發位置。[^1]
- **Lesion quotient（病灶商數）——量化鑑別工具**：定義為**T2WI上低訊號結節面積 ÷ 對比後T1WI上同病灶面積**。研究顯示（32病灶）：**腫瘤復發病灶商數 >0.6**，**放射線壞死病灶商數 <0.3**——即**腫瘤復發的病灶商數較放射線壞死「高」**（非「放射線壞死較高」）。[^1]
...
> **B為偽**——腫瘤復發的lesion quotient（病灶商數）較放射線壞死「高」（復發>0.6、放射線壞死<0.3），非放射線壞死較高。可依原發腫瘤位置與照野推測、腦室旁強化灶典型提示放射線引起皆為放射線壞死特徵。[^1]

## 考題
```dataview
list from #交換 where contains(concepts, "radiation-necrosis-vs-tumor-recurrence")
```

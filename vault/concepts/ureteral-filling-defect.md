---
concepts: [ureteral-filling-defect]
name: Ureteral Filling Defect (Differential)
subspecialty: [ABD]
aliases:
  - ureteral filling defect
  - ureteric filling defect
  - 輸尿管充盈缺損
dateRev: 2026-06-15
same:
  - "[[ectopic-ureter]]"
---

# ureteral-filling-defect

**輸尿管充盈缺損最常見的原因是『結石（calculi）』；腫瘤（urothelial/TCC）是最常見的『腫瘤性』充盈缺損但整體頻率遠低於結石。** 判讀分水嶺一：**先想結石**（最常見、可 CT 平掃確認）；判讀分水嶺二：再依單發/多發鋪開鑑別（CUPS MET：clot、stone、polyp、sloughed papilla、metastasis、endometriosis、TB、urothelial carcinoma）。

## Summary
- **最常見原因＝結石（calculi）**（標準 uroradiology 教學；題目正解）。[^1][^2]
- **腫瘤**：urothelial carcinoma（原 TCC）是最常見的**腫瘤性**充盈缺損，但整體頻率低於結石；papilloma 罕見。[^1][^2]
- **血塊（clot）**：續發於出血（腫瘤/外傷/抗凝），會隨時間變化、可移動。[^1]
- **單發充盈缺損鑑別（CUPS MET）**：Clot、Ureteric stone、Polyp、Sloughed papilla、Metastasis、Endometriosis、TB、urothelial carcinoma。[^1]
- **多發充盈缺損**：多發結石（steinstrasse）、ureteritis cystica、多灶 urothelial carcinoma、血塊、血管壓跡等。[^1]

> [!note] 考點排序
> 「最常見的輸尿管充盈缺損原因」= **結石**，不是 TCC（TCC 只是最常見的『腫瘤性』充盈缺損）。[^1][^2]

### 參考來源
[^1]: *Solitary / multiple filling defects of the ureter (differential)*. Radiopaedia.org。單發 CUPS MET（clot、ureteric stone、polyp、sloughed papilla、metastasis、endometriosis、TB、urothelial carcinoma）；多發含多發結石、ureteritis cystica、多灶 UC 等（實際查證 accessed 2026-06-15）。
[^2]: Potenta SE, D'Agostino R, Sternberg KM, et al. *CT Urography for Evaluation of the Ureter*. RadioGraphics. 2015;35(3):709-726. https://doi.org/10.1148/rg.2015140209 （題目所引，回顧輸尿管 filling defect；經 PubMed 取得書目 PMID 25815907。「結石為最常見」為標準教學，未於該文逐字核實）。

## 題目
> [!question]- 輸尿管充盈缺損最常見的原因？
> **結石（calculi）**。urothelial carcinoma（原 TCC）為最常見的『腫瘤性』充盈缺損但整體頻率低於結石；papilloma 罕見、clot 續發於出血。[^1][^2]

## 考題
```dataview
list from #交換 where contains(concepts, "ureteral-filling-defect")
```

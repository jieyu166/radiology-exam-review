...
## 放射科醫師影像判讀重點
- **穿透率速算（考試核心）**：n 個 HVL 後穿透分率＝(1/2)ⁿ；1 HVL=50%、2=25%、3=12.5%、4=6.25%、**5≈3.1%（≈3%）**、6≈1.6%、7≈0.8%——**5 HVL 對應約 3%**為本題關鍵，切勿誤選 50%。[^1]
- **量測幾何**：HVL 須於 **narrow-beam（窄束）幾何**下量測最準；寬束因散射線混入會**高估**穿透、低估衰減，故品保量測要準直並拉開偵測器距離。[^2]
- **束質 vs 半衰期（易混淆點）**：HVL 反映 X 光束之**束質／硬度（能量分布）**，單位為 mm Al；與放射性核種之**半衰期（時間衰變）**是不同概念，勿混為一談。[^3]
- **濾片與臨床品保**：加鋁／銅濾片可濾除低能光子→**增大 HVL、降低病人皮膚劑量、改善束之單能性（homogeneity coefficient）**；乳房攝影與診斷 X 光機之 HVL 有法定最低值，是機器品保與輻防查核項目。[^3]
- **多能束 beam hardening**：實際多能束每通過一層物質後低能光子先被吸收，束變硬，故**後續各層 HVL 依序增大**（非固定），與理想單能束的固定 HVL 不同。[^2][^3]

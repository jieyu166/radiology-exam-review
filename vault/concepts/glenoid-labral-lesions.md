---
concepts: [glenoid-labral-lesions]
name: Glenoid Labral Lesions & Normal Variants
subspecialty: [MSK]
aliases:
  - glenoid labrum lesions
  - Bankart
  - Perthes
  - ALPSA
  - Buford complex
  - 肩盂唇傷害
dateRev: 2026-07-02
---

# glenoid-labral-lesions

**前下盂唇損傷（Bankart／Perthes／ALPSA）是肩前脫位的結果;但 Buford complex 是『正常變異』（前上盂唇缺如＋中盂肱韌帶呈索狀增厚）,不是傷害,勿誤判為撕裂。** 判讀分水嶺：**Bankart＝前下盂唇撕脫（±骨性 bony Bankart）;Perthes＝前下盂唇撕脫但骨膜完整、盂唇仍貼附（非移位）、易漏需 ABER 姿勢;ALPSA＝盂唇骨膜袖套向內下移位且內側化癒合;Buford complex＝前上（1-3 點鐘）盂唇缺如＋索狀中盂肱韌帶,屬正常變異。**

## Summary
- **Bankart lesion**：前下盂唇自盂緣撕脫（前脫位典型）;**bony Bankart**＝合併前下盂緣骨折。[^1]
- **Perthes lesion**：前下盂唇撕脫但**骨膜完整、盂唇未明顯移位**仍貼附 → MRI 易漏,**ABER（外展外旋）姿勢**較易顯現。[^1]
- **ALPSA（anterior labroligamentous periosteal sleeve avulsion）**：盂唇-骨膜袖套**向內下移位並內側化癒合**。[^1]
- **Buford complex（正常變異,非傷害）**：**前上盂唇（1-3 點鐘）缺如＋中盂肱韌帶呈索狀（cord-like）增厚**;勿誤判為盂唇撕裂或 SLAP。[^1]

> [!note] 考點：關於肩部盂唇,何者「為非（不是傷害）」?
> 「**Buford complex**」為非——它是**正常解剖變異**（前上盂唇缺如＋索狀中盂肱韌帶）,不是盂唇傷害。Bankart、Perthes、ALPSA 皆為前下盂唇損傷型態。[^1]

## Summary — 正常變異（Normal Variants）
- **Buford complex**：前上盂唇（1-3 點鐘）缺如 + 中盂肱韌帶索狀增厚——最常被考的正常變異。[^1]
- **Sublabral foramen**：前上盂唇與盂緣間的局部缺損（foramen），為正常變異。[^2]
- **Superior sublabral sulcus (recess)**：上盂唇基部與盂緣間的溝槽/凹陷，須與 SLAP tear 鑑別（正常 recess 邊緣光滑、平行盂緣走行）。[^2]
- **Pseudo-SLAP lesion**：上盂唇表面纖維化或磨損模擬 SLAP 外觀，非真正撕裂。[^2]
- **「Subglenoid recess」非公認正常變異**——不在標準正常變異清單中。[^2]

### 參考來源
[^1]: 官方 2017 詳解；Tuite MJ et al. / Radiology 2016;278(2):316-332（盂唇損傷與變異綜述）：Bankart/bony Bankart、Perthes（骨膜完整、需 ABER）、ALPSA（內側化癒合）為前下盂唇損傷;Buford complex（前上盂唇缺如＋索狀中盂肱韌帶）為正常變異。
[^2]: 2019 交換考詳解；Dahnert Radiology Review Manual 7th ed., pp. 32-33。

## 題目
> [!question]- 有關棒球投手肩部盂唇傷害,下列何者「為非」? (2017-203)
> 「**Buford complex（A）**」為非——它是**正常解剖變異**（前上盂唇缺如＋索狀中盂肱韌帶），不是盂唇傷害。Bankart、Perthes、ALPSA 皆為前下盂唇損傷。[^1]

## 考題
```dataview
list from #交換 where contains(concepts, "glenoid-labral-lesions")
```

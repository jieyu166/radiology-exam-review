---
concepts: [pres]
name: PRES (Posterior Reversible Encephalopathy Syndrome)
subspecialty: [NR]
aliases:
  - PRES
  - posterior reversible encephalopathy syndrome
  - reversible posterior leukoencephalopathy
  - 後部可逆性腦病變
dateRev: 2026-06-15
---

# pres

> [!info] Summary 圖・待醫師覆核
> ![Summary 圖：PRES：典型影像與背景](https://img-hosting.jieyu166.workers.dev/i/yjAetfW.png)
> [開啟原尺寸圖片](https://img-hosting.jieyu166.workers.dev/i/yjAetfW.png)
>
> 教學摘要圖，內容仍待醫師覆核。


**PRES 的因果鏈：血壓失調 → 高灌流破壞血腦屏障 → vasogenic edema 在後頂枕葉堆積 → T2/FLAIR 亮、DWI 不受限（可逆）。** 常見誘因「高血壓、子癇、免疫抑制藥物（tacrolimus/cyclosporine 等）、腎病」；糖尿病不在此列（考試陷阱）。判讀分水嶺一：**DWI 不受限 ＋ ADC 升高 → vasogenic edema → PRES（區別急性梗塞的 DWI 受限）**；判讀分水嶺二：**移除誘因多可逆；DM 不是典型危險因子**。

## Summary

- **重點**: 機轉：急性血壓變化超出腦循環自我調節 → 高灌流 → 血腦屏障破壞 → vasogenic edema（典型 parieto-occipital 對稱，T2/FLAIR 高訊號、DWI 多不受限）。 [^1]
- **重點**: 常見危險因子： [^1]
- **重點**: 嚴重高血壓。 [^1]
- **重點**: 子癇前症/子癇（pre-eclampsia/eclampsia）、產後。 [^1]
- **重點**: 免疫抑制/藥物毒性：tacrolimus、cyclosporine、bevacizumab、methotrexate、vincristine、cisplatin、cyclophosphamide、corticosteroids、rituximab 等。 [^1]
- **重點**: 腎病：急性腎絲球腎炎、慢性腎功能不全。 [^1]
- **重點**: 非典型危險因子＝糖尿病（DM）（題目正解）。 [^1]
- **重點**: 多為可逆（移除誘因/控制血壓後）。 [^1]

## 影像診斷
**核心：雙側、相對對稱的 vasogenic edema，好發 parieto-occipital 皮質下白質；DWI 多『不受限』、ADC 升高（與急性梗塞區分）。**
### 分布型態
- **最常見 parieto-occipital**（後循環分水嶺）；常**雙側、對稱**、以**皮質下白質**為主（皮質亦可）。[^1]
- 名稱實為 misnomer——**可延伸/侵犯後腦以外**：上額溝（superior frontal sulcus）、額/顳葉、**腦幹、基底節、小腦**（atypical 部位）。[^1]
- 三種常見分布型（Bartynski）：①holohemispheric watershed ②superior frontal sulcus ③dominant parieto-occipital。[^1]
### CT
- 受侵區域**低密度（hypoattenuating）**。[^1]
### MRI
- **T2/FLAIR 高訊號**（vasogenic edema）於受侵區。[^1]
- **DWI 多正常/不受限、ADC 升高（vasogenic）**；少數呈 T2 shine-through 或**真正擴散受限 → 提示梗塞/預後較差**。[^1]
- **強化**：型態多變、約部分病例可見；**有無強化不改變診斷或預後**。[^1]
- **出血**：可見（微出血/實質出血/SAH）。[^1]
### 其他
- **DSA/MRA**：可見**血管收縮/vasospasm**（與 RCVS 重疊）。[^1]

> [!warning] 與急性梗塞的關鍵區分
> PRES 是 **vasogenic edema → DWI 不受限、ADC 升高**；急性梗塞為 cytotoxic edema（DWI 受限、ADC 下降）。PRES 若出現真正擴散受限，提示已進展為梗塞、預後較差。[^1]

> [!note] 考點：何者「非」常見危險因子？(2016)
> **DM（糖尿病）**。常見者：高血壓、子癇、免疫抑制/藥物、腎功能不全。[^1][^2]

## 補充危險因子（2018交換）
- **鈣調神經磷酸酶抑制劑(calcineurin inhibitors，如tacrolimus/cyclosporine)** 與**古柯鹼、安非他命**皆可致PRES。詳見 [[toxic-metabolic-leukoencephalopathy]] 之其他藥物性中樞神經病變（chasing the dragon、苯妥英顱骨增厚、TPN錳沉積等）。[^3]

### 參考來源（續）
[^3]: *RadioGraphics* 2012;32:701-719（2018 交換考題詳解 p.137 引用；Tier 1 影像期刊）。鈣調神經磷酸酶抑制劑、古柯鹼、安非他命皆可致PRES。

### 參考來源
[^1]: *Posterior reversible encephalopathy syndrome*. Radiopaedia.org。誘因含嚴重高血壓、產後 eclampsia/pre-eclampsia、急性腎絲球腎炎、藥物毒性（tacrolimus/cyclosporine/化療/類固醇等）；機轉為自我調節失衡致 vasogenic edema。影像：parieto-occipital 為主、可延伸後腦以外（額/腦幹/基底節/小腦）、三型分布；CT 低密度；T2/FLAIR 高訊號、DWI 多不受限/ADC 升高（少數真受限提示梗塞）、強化多變、可出血、DSA 可見 vasospasm（實際查證 accessed 2026-06-16）。
[^2]: Hinchey J, Chaves C, Appignani B, et al. *A reversible posterior leukoencephalopathy syndrome*. N Engl J Med. 1996;334:494-500. https://doi.org/10.1056/NEJM199602223340803 （題目所引原始描述；危險因子含高血壓、子癇、免疫抑制治療、腎病）。

## 考題
```dataview
list from #交換 where contains(concepts, "pres")
```

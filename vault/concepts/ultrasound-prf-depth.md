...
## 技術要點
- **PRF 之物理限制**：機器必須等最深回波返回才能發下一脈衝，故 **PRF = c / (2R)**；深度 R 越大、來回時間越長、每秒能發脈衝數（PRF）越少。[^1][^2]
- **深度—PRF 反比**：反解得 R = 0.5 × (c / PRF)，深度與 PRF 成反比；此為考題計算與觀念判斷之核心關係式。[^1]
- **與深度無關之三個干擾選項**：spatial pulse length（SPL＝波長×每脈衝週期數）、pulse duration（PD＝週期數×週期，典型 <1 μs）、wavelength（λ＝c/頻率）皆由探頭頻率與介質決定，不隨成像深度改變。[^1]
- **連帶 frame rate 效應**：深度↑ → PRF↓ →每條掃描線取樣更久 → **frame rate 下降**，即時性變差；欲維持高 frame rate 須縮小深度或掃描寬度。[^1][^2]
- **臨床調控（knobology）**：增加成像深度時，機器自動降低 PRF；操作者若需同時兼顧深部與時間解析度，應權衡深度、寬度與 frame rate。[^2]

...
**CAC 分數用非顯影、心電圖閘控 CT 量化冠狀動脈鈣化，是無症狀者心血管風險的非侵入性指標。** 兩個判讀錨點：**CAC = 0 是強力的「負向風險因子」（power of zero）**，事件率低；**CAC ≥ 400 為高風險**，且重度鈣化的 blooming 偽影會顯著降低後續 CCTA 的 specificity。計分採 Agatston 法：鈣化定義為 ≥ 130 HU 且面積 ≥ 1 mm²，面積乘以 HU 加權係數後加總。記憶鉤：**「0 分 = 放心 5 年；400 分 = 高危險且 CCTA 難判讀」**。
...
- **鈣化判定的雙門檻**：僅將 **≥130 HU 且面積 ≥1 mm²** 的病灶計入；孤立單一像素高密度、影像雜訊或運動假影不應計分，否則高估分數。[^1]
- **逐血管掃描分區**：分別檢視 LM、LAD、LCx、RCA 四條冠狀動脈，各病灶 Agatston 值加總為總分；判讀時須避免把主動脈壁鈣化、瓣膜/瓣環鈣化（mitral annular、aortic valve）或縱膈鈣化淋巴結誤計入冠狀動脈。[^1]
- **分數分層對應風險**：0 / 1–99 / 100–399 / ≥400（mild/moderate/severe）；報告 CAC = 0 應強調其強力負向預測價值（power of zero），CAC ≥ 400 應提示高風險並提醒後續 CCTA 判讀受限。[^1]
- **重度鈣化的下游影響（判讀陷阱）**：CAC > 400 之 blooming 假影會顯著降低後續 CCTA 的 specificity/PPV；報告高 CAC 時宜註記此限制，供臨床決定是否改採功能性檢查。[^1]
- **運動假影辨識**：心律不整或高心率造成的階梯狀錯位（stair-step artifact）可假性增加或減少鈣化面積，判讀時須辨識並於報告註明品質受限。[^1]

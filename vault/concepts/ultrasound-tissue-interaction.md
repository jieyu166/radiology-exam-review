...
## 放射科醫師影像判讀重點
- **impedance 決定界面回音強度**：相鄰組織聲阻抗差（Z＝密度×聲速）越大，界面反射越強、穿透越少——組織／氣體、組織／骨界面因巨大失配幾近全反射，造成其後方**聲影（acoustic shadowing）**與無法評估的死角，是判讀腸氣與骨後方結構的物理根源。[^1][^2]
- **scattering 決定實質臟器 echogenicity 與 speckle**：肝、腎、脾各自的均勻回音紋理來自小於波長之小界面 backscatter（散射），非鏡面反射；判讀臟器彌漫性病變（如脂肪肝回音增強）本質是散射特性改變。[^1][^2]
- **鏡面反射的角度依賴（anisotropy）**：大而平滑界面（橫膈、膀胱壁、骨皮質、肌腱）產生 specular reflection，回音強但**須探頭近垂直方能接收**；傾斜即訊號掉落，易被誤判為結構缺損或病灶——判讀肌腱時尤須警覺此偽影。[^2]
- **折射造成定位誤差與邊緣陰影**：超音波斜射過兩側聲速不同界面時依 Snell's law 改變方向，機器仍假設直行故將回波錯位（mispositioning）；大曲面（囊腫、膽囊、胎兒頭顱）邊緣兩側形成沿軸向延伸的 **edge shadow**，勿誤判為結石或病灶。[^2]
- **判讀陷阱整合**：「大邊界靠阻抗、細紋理靠散射、平滑面認角度、曲面邊緣防折射」——四類物理效應各對應不同影像表現與偽影，判讀前先辨機制再下診斷。[^1][^2]

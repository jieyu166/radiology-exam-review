---
concepts: [iodinated-contrast-media]
name: Iodinated Contrast Media (Classification, Osmolality, Distribution)
...
- **分類與代表藥**：[^1]
  ...
  | 類別 | 滲透壓 | 代表藥 |
  |---|---|---|
  | Ionic monomer（HOCM）| 高(~1400–2000) | Conray（iothalamate meglumine）|
  | Non-ionic monomer（LOCM）| 低(~600–850) | Omnipaque(iohexol)、Isovue(iopamidol)、Optiray(ioversol)、Ultravist(iopromide) |
  | Non-ionic dimer（IOCM）| 等滲(~290) | **Visipaque（iodixanol）** |
...
- **鞘內(intrathecal)/脊髓攝影**：**只准特定低濃度非離子單體**（如 Omnipaque 240/300、Iopamidol/Iopamiro 300）;**高濃度（Omnipaque 350/370、Visipaque）禁用於鞘內**（神經毒性）。[^1]
- **X 光物理**：碘**原子序高(Z=53)＋成本低** → 良好對比材料;增加 SNR **也**改善 CNR（對比不影響背景 SI）。[^1]
- **保存/小兒**：保溫箱 37°C（放置儘快用完、最久 ~1 個月）、庫存室溫 15–30°C;**溫度越高黏性越低**（故加溫）。小兒約 **1.5–2 cc/kg**,CVC 多不適合注射。[^3]
...
- ✅ **本考點答案已裁決（醫師 2026-07-11：C→D）**：題幹問「120 kVp 主動脈 CTA 之常規碘負荷量」，**conventional 120 kVp = 400 mgI/kg（選項 D）**，而 **200 mgI/kg（選項 C）係 double-low（70 kVp）之協定**。原匯入答案鍵標 C 與官方詳解自載的 400 mgI/kg 矛盾，經醫師裁決更正為 **D（400 mgI/kg）**（json/卡片 correctAnswer 已為 D）。
...
> **答案 D（400 mgI/kg）**（原匯入答案鍵標 C，經醫師 2026-07-11 裁決更正為 D）——官方詳解自載「conventional 120 kVp 主動脈 CTA＝**400 mgI/kg（D）**」，而 **200 mgI/kg（C）為 double-low CTA（70 kVp＋低碘）之協定**，與題幹「120 kVp」不符。物理上：降低管電壓趨近碘 k-edge（33.2 keV）使碘衰減上升，70 kVp 較 120 kVp 高約 51%，故 DLCTA 可將碘負荷約減半至 ~200 mgI/kg。[^7][^8]

---
concepts: [salter-harris]
name: Salter-Harris Classification
subspecialty: [MSK, PED]
aliases:
  - Salter-Harris classification
  - growth plate fracture
  - 生長板骨折分類
dateRev: 2026-06-17
---

# salter-harris

**Salter-Harris 生長板骨折五型（SALTR 記憶）：I Slipped（僅經生長板）、II Above（經生長板+幹骺端,『最常見 ~75%』）、III Lower（生長板+骨骺）、IV Through（貫穿骨骺-生長板-幹骺端）、V Rammed（壓砸,預後最差）。最常見＝Type II。** 判讀分水嶺一：**type II ~75% 最常見**;判讀分水嶺二：**型號越高（III–V 累及骨骺/關節面）預後越差**。

## Summary
- **Type I（Slipped）**：骨折線僅經生長板;預後好。[^1]
- **Type II（Above）**：經生長板並向上經**幹骺端**;**最常見（~75%,by far）**（2016-189 正解 B）;預後好。[^1]
- **Type III（Lower）**：生長板＋向下經**骨骺**（累及關節面）。[^1]
- **Type IV（Through）**：貫穿骨骺-生長板-幹骺端。[^1]
- **Type V（Rammed）**：生長板**壓砸傷**;預後最差、易生長停滯。[^1]

> [!note] 考點：最常見的 Salter-Harris 型?
> **Type II（~75%,by far the most common）**——經生長板+幹骺端。[^1]

### 參考來源
[^1]: *Salter-Harris classification*. Radiopaedia.org（實際查證 accessed 2026-06-17）：**type II ~75%「by far the most common」**（生長板+幹骺端）;I 僅生長板、III 生長板+骨骺、IV 貫穿、V 壓砸（預後最差）;記憶 SALTR。

## 題目
> [!question]- 最常見的 Salter-Harris 骨折型?
> **Type II（~75%）**——經生長板並向上經幹骺端。[^1]

> [!question]- 何種敘述最能描述侵犯生長板(physis)的骨折? (2017-046)
> **縱向(longitudinal/vertical)骨折比橫向(horizontal)骨折更易形成骨橋(bridge)** → 生長停滯風險高(SH III/IV 跨生長板)。橫向(SH I/II)bridge 約 25%;不建議常規打 gadolinium(B 誤)、年幼者預後較差非年長(C 誤)、粉碎程度影響預後(D 誤)。[^1]

## 考題
```dataview
list from #交換 where contains(concepts, "salter-harris")
```

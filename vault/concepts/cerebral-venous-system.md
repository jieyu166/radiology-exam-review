---
concepts: [cerebral-venous-system]
name: Cerebral Venous System (Sinuses & Deep/Superficial Veins)
subspecialty: [NR]
aliases:
  - cerebral venous system
  - dural venous sinuses
  - deep cerebral veins
  - vein of Galen
  - 腦靜脈系統
dateRev: 2026-06-17
same:
  - "[[cerebral-deep-venous-cortex]]"
---

# cerebral-venous-system

**腦靜脈回流有兩套系統：淺部（皮質靜脈→上矢狀竇、橫竇）和深部（內大腦靜脈 + Rosenthal 基底靜脈 → vein of Galen）。** 下矢狀竇（ISS）沿大腦鐮下緣走，最終與 vein of Galen 匯合形成 straight sinus。考點陷阱：Trolard（superior anastomotic）與 Labbé（inferior anastomotic）是淺部吻合靜脈，外表名氣大、實為淺部，不算深部系統。記憶鉤：「ISS 從下走到底找 Galen→直竇；Trolard 和 Labbé 是淺部的橋梁，別歸錯隊伍」。

## Summary
- **下矢狀竇（ISS）**：沿大腦鐮下緣走,**與 vein of Galen 匯合形成 straight sinus**（2016-176 正解 C）。[^1]
- **深部靜脈系統**：[^2]
  - **Internal cerebral veins（ICV）**(A 深部)
  - **Basal veins of Rosenthal（BVR）**(B 深部)
  - **Vein of Galen（great cerebral vein）**(C 深部)
- **淺表（cortical）吻合靜脈**：**superior anastomotic vein of Trolard**（連淺中大腦靜脈↔上矢狀竇）、inferior anastomotic vein of Labbé（↔橫竇）;故 **Trolard『不』屬深部靜脈系統**＝2016-177 正解 D。[^2]

> [!note] 考點
> ISS 與 **vein of Galen** 匯合→straight sinus（176 答 C）。深部靜脈＝ICV/BVR/Galen;**Trolard 為淺表吻合靜脈**（177 答 D）。[^1][^2]

### 參考來源
[^1]: *Inferior sagittal sinus*. Radiopaedia.org（實際查證 accessed 2026-06-17）：沿大腦鐮下緣,與 great cerebral vein（Galen）匯合形成 **straight sinus**。
[^2]: 腦深部靜脈系統＝internal cerebral veins、basal veins of Rosenthal、vein of Galen（標準神經解剖）;**Trolard（superior anastomotic）與 Labbé（inferior anastomotic）為淺表 cortical 吻合靜脈**。見 [[cerebral-deep-venous-cortex]]。

## 考題
```dataview
list from #交換 where contains(concepts, "cerebral-venous-system")
```

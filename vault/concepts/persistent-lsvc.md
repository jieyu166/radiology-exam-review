---
concepts: [persistent-lsvc]
name: Persistent Left Superior Vena Cava (PLSVC)
subspecialty: [CV]
aliases:
  - persistent left superior vena cava
  - PLSVC
  - left-sided SVC
  - 永存左上腔靜脈
dateRev: 2026-06-16
---

# persistent-lsvc

**PLSVC 是最常見的胸腔先天靜脈異常，核心記憶：「左側縱膈血管影 → 追到 dilated coronary sinus → 問它流去哪」。** 約 90% 經 coronary sinus 引流入右心房（無症狀偶然發現）；少數直接入左心房才產生右→左分流。判讀分水嶺一：**引流路徑決定臨床意義**——coronary sinus → 右心房（最常見、無分流）vs 直接入左心房（少數、可致反常栓塞）；判讀分水嶺二：**dilated coronary sinus 是影像線索，見之需排除 PLSVC**。

## Summary
- **最常見的胸腔先天靜脈異常**。[^1]（選項 A 正確）
- **胚胎來源**：未退化之 **left anterior cardinal vein（＋left common cardinal vein）** 持續存在。[^1]（選項 D 正確）
- **引流**：**多數經 coronary sinus → 右心房**（文獻約 ~90%+／題目引 92%）；**少數直接入左心房** → 右→左分流（可致反常栓塞、發紺）。[^1]
- **影像招牌**：**dilated coronary sinus**（最常見線索）；左上縱膈見與主動脈弓平行下行的靜脈。[^1]（選項 C 正確）
- 臨床意義：多無症狀（偶然發現）；左側中心靜脈導管/節律器導線、心臟手術前須辨識。[^1]

> [!note] 考點：何者「錯」？
> 「**通常匯入左心房**」錯——PLSVC **通常經 coronary sinus → 右心房**（~90%+）；入左心房屬少數。其餘（最常見胸腔先天靜脈異常、dilated coronary sinus、源自 left anterior/common cardinal vein）皆正確。[^1]

### 參考來源
[^1]: *Left-sided superior vena cava*. Radiopaedia.org。最常見胸腔先天靜脈異常；源自 persistent left anterior cardinal vein；多引流入 coronary sinus（→右心房），少數入左心房（右→左分流）；dilated coronary sinus 為線索（實際查證 accessed 2026-06-16）。題目另引 Sonavane SK, et al. *Comprehensive Imaging Review of the Superior Vena Cava*. RadioGraphics 2015;35(7):1873-92. https://doi.org/10.1148/rg.2015150056 。

## 題目
> [!question]- 關於 persistent left SVC，下列何者「錯誤」？
> 「**通常匯入左心房**」錯——通常經 **coronary sinus → 右心房**（~90%+），少數才入左心房（右→左分流）。最常見胸腔先天靜脈異常、dilated coronary sinus、源自 left anterior/common cardinal vein 皆正確。[^1]

## 考題
```dataview
list from #交換 where contains(concepts, "persistent-lsvc")
```

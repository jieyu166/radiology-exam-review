---
concepts: [foot-ankle-soft-tissue-masses]
name: Foot & Ankle Soft-Tissue Masses — MRI Signal
subspecialty: [MSK]
aliases:
  - foot and ankle mass MRI
  - plantar fibromatosis MRI
  - Morton neuroma signal
  - 足踝軟組織腫塊
dateRev: 2026-07-10
---

# foot-ankle-soft-tissue-masses

**足踝軟組織腫塊靠MRI訊號『指紋』分類——homogeneous low T1 AND low T2 且會強化，最典型的是『纖維性腫塊(plantar fibromatosis / fibroma of tendon sheath)』，因含成熟膠原(mature collagen)。** 判讀口訣:T1、T2皆低訊號＝想fibrous tissue;含未成熟纖維/纖維母細胞者訊號可呈中等。其他腫塊的訊號各有指紋,是鑑別關鍵。

## Summary
- **Plantar fibromatosis / 纖維性腫塊(本題答案)**:含**成熟膠原(mature collagen)**→ **T1與T2皆均勻低訊號且強化**;含未成熟纖維組織者訊號可呈中等。[^1]
- **Morton's neuroma**:T1**與肌肉等訊號(isointense)**、T2**低訊號**,強化程度不一。[^1]
- **Lipoma**:脂肪訊號→ **T1與T2皆高訊號**,脂肪抑制序列上呈低訊號。[^1]
- **Ganglion cyst**:**T1低、T2高**訊號;通常無中央強化(僅薄邊環狀強化);若見中央強化須排除惡性。[^1]
- **Hemangioma**:因含血管、脂肪、纖維組織→ T1/T2呈**混雜(mixed)訊號**;血管部分均勻強化。[^1]
- 附註:緻密鈣化腫塊可T1/T2皆低訊號,但通常不強化。[^1]

## 放射科醫師影像判讀重點
- **判讀主軸（本題核心）**：足踝軟組織腫塊靠 MRI 訊號「指紋」分類——**T1 與 T2 皆均勻低訊號且會強化**最典型為**纖維性腫塊（plantar fibromatosis／fibroma of tendon sheath）**，因含**成熟膠原（mature collagen）**；含未成熟纖維／纖維母細胞成分者訊號可呈中等，是判讀時的變異。[^1]
- **各腫塊訊號鑑別**：Morton's neuroma T1 與肌肉等訊號、T2 低訊號、強化不一；Lipoma T1/T2 皆高訊號、脂肪抑制序列變低；Ganglion cyst T1 低／T2 高、通常僅薄邊環狀強化；Hemangioma 因含血管／脂肪／纖維呈 T1/T2 混雜訊號、血管部分均勻強化——訊號組合是分野關鍵。[^1]
- **部位定位輔助**：plantar fibromatosis 沿足底腱膜（plantar aponeurosis）呈結節狀，Morton's neuroma 位於蹠骨間隙（好發第 3–4 蹠骨間），部位＋訊號合看比單看訊號更可靠。[^1]
- **判讀陷阱**：**Ganglion cyst 若見中央強化須排除惡性**（正常僅薄邊環狀強化）；緻密鈣化腫塊雖 T1/T2 皆低但**通常不強化**，與會強化的纖維性腫塊區別；勿把纖維性腫塊的低 T2 誤讀為單純鈣化或含鐵血黃素沉積。[^1]
- **流行病學（考點）**：**足踝最常見的軟組織腫塊是 ganglion cyst（腱鞘囊腫）**，屬整體最常見的軟組織腫塊；影像上 T1 低／T2 高、無中央強化（僅薄邊環狀強化），部位常沿關節囊／腱鞘。[^1][^2]
- **檢查選擇（考點）**：**MRI 為足部軟組織腫瘤『特徵化（characterization）』最具特異性的檢查**——高軟組織對比可依 T1／T2／脂肪抑制／強化型態辨別脂肪、纖維、囊液、血管等成分；US 適合初步分辨囊實性但特異度較低、CT 對軟組織對比差、PET 主用於代謝／分期而非組織特徵化。[^1]

> [!note] 考點:homogeneous low T1與low T2訊號且強化,最典型為何種足踝腫塊?
> **Plantar fibromatosis(C)**——纖維性腫塊含成熟膠原,T1/T2皆低訊號且強化。Lipoma(T1/T2高)、Ganglion(T1低T2高)、Hemangioma(混雜訊號)、Morton neuroma(T1等肌肉T2低)訊號皆不符。[^1]

### 參考來源
[^1]: *Imaging Evaluation of Foot and Ankle Pathology*. AJR 2008;190:S18-S22. doi:10.2214/AJR.07.7023（2018 交換考題詳解 p.344 引用；Tier 1 期刊）。纖維性腫塊(plantar fibromatosis/fibroma of tendon sheath)含成熟膠原呈T1/T2低訊號且強化;Morton neuroma T1等肌肉/T2低訊號;lipoma脂肪訊號;ganglion T1低T2高;hemangioma混雜訊號(另參RadioGraphics 1998;18:1481、Semin Musculoskelet Radiol 2005;9:227)。此文並指出 ganglion cyst 為足踝最常見軟組織腫塊、MRI 為軟組織腫瘤特徵化最具特異性之檢查。
[^2]: *Ganglion cyst*. Radiopaedia.org（實際查證 accessed 2026-07-06）：ganglion cyst 為**最常見的軟組織腫塊**，好發年輕女性，約 70–80% 位於手／腕，足踝亦為好發部位；為含膠狀液之囊性病灶（T1 低、T2 高），與關節囊／腱鞘相通。

## 題目
> [!question]- An MRI appearance of homogeneous low T1- and T2-weighted signal with enhancement is MOST typical of which foot and ankle mass? (2018-069)
> **Plantar fibromatosis（C）**——纖維性腫塊含成熟膠原,T1/T2皆均勻低訊號且強化。Lipoma(A,脂肪高訊號)、Ganglion cyst(B,T2高)、Hemangioma(D,混雜)、Morton neuroma(E,T1等肌肉/T2低)訊號皆不符。[^1]

> [!question]- Which soft-tissue mass is MOST common in foot and ankle? (2022-460)
> **Ganglion cyst（C，關鍵）**——ganglion cyst（腱鞘囊腫）為足踝最常見的軟組織腫塊，影像呈 T1 低／T2 高、無中央強化。Morton's neuroma(A)、PVNS(B)、plantar fibromatosis(D) 皆較少見。詳見 [[foot-ankle-soft-tissue-masses]]。[^1][^2]

> [!question]- Which is the most specific examination for characterization of a soft-tissue neoplasm in the foot? (2022-461)
> **MRI（A，關鍵）**——MRI 高軟組織對比，可依 T1／T2／脂肪抑制／強化型態辨別脂肪、纖維、囊液、血管等成分，為足部軟組織腫瘤特徵化最具特異性之檢查。CT(B) 軟組織對比差、PET(C) 主用於代謝／分期、Sonography(D) 適合初步囊實性判別但特異度較低。詳見 [[foot-ankle-soft-tissue-masses]]。[^1]

> [!question]- An MRI appearance of homogeneous low T1- and T2-weighted signal with enhancement is MOST typical of which foot and ankle mass? (2021-334)
> **D（Plantar fibromatosis）**——纖維性腫塊含成熟膠原，呈 T1、T2 皆均勻低訊號且會強化。A Morton's neuroma（T1 等肌肉、T2 低）、B Lipoma（T1/T2 高）、C Ganglion cyst（T1 低 T2 高、僅薄邊環狀強化）、E Hemangioma（混雜訊號）訊號組合皆不符。

## 考題
```dataview
list from #交換 where contains(concepts, "foot-ankle-soft-tissue-masses")
```

---
concepts: [renal-artery-stenosis]
name: Renal Artery Stenosis (Doppler & Revascularization)
subspecialty: [US, IR]
aliases:
  - renal artery stenosis
  - RAS
  - renovascular hypertension
  - resistive index
  - 腎動脈狹窄
dateRev: 2026-06-18
---

# renal-artery-stenosis

**腎動脈狹窄 Doppler 的因果鏈：狹窄→局部 PSV ↑（>180 cm/s）→下游水流緩慢（tardus-parvus，AT >0.07 s）；但萎縮小腎（<8 cm）＋高實質 RI（>0.8）代表末期腎硬化，修通血管也無效，即使看到狹窄也不介入。** 判讀分水嶺一：**PSV >180–200 cm/s、RAR >3.5、AT >0.07 s → 血流動力學顯著狹窄**；判讀分水嶺二：**小萎縮腎＋高 RI → 腎實質已纖維化，血管重建無效 → 不介入**。

## Summary
- **狹窄直接徵象**：狹窄處 **PSV >180–200 cm/s**、renal-aortic ratio (RAR) >3.5、aliasing。[^1]
- **下游間接徵象**：**tardus-parvus 波形**（收縮上升緩慢）、acceleration time >0.07 s、AI 降低。[^1]
- **介入適應與排除**：[^1]
  - **小而萎縮的腎（雙側 5–6 cm，<8 cm）＋ 實質 RI 高（~0.9，>0.8）** → 提示**末期腎硬化／腎實質病變**,血管重建難改善 → **不介入**。
  - 題例 PSV 130／160 cm/s **未達 >180 狹窄閾值**（非血流動力學顯著狹窄）,加上雙側小腎、RI 0.9 → **無介入適應症**。

> [!note] 考點：雙側小腎（5–6 cm）、RI 0.9、PSV 130／160 cm/s,該如何處置?
> **不介入（No intervention）**。PSV 未達狹窄閾值（>180）、且雙側萎縮腎＋高 RI（腎硬化）→ 血管重建無益。[^1]

### 參考來源
[^1]: *Renal artery stenosis*. Radiopaedia.org（實際查證 accessed 2026-06-18）：「increased **peak systolic velocity (PSV): some advocate 180 cm/s**」、RAR、tardus-parvus、acceleration time >0.07 s。介入排除（小腎＋高實質 RI 預示血管重建無效）依 Morgan R & Waltzer E. Handbook of Angioplasty and Stenting Procedures（題目所引）與既有腎血管文獻。

## 考題
```dataview
list from #交換 where contains(concepts, "renal-artery-stenosis")
```

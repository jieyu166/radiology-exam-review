...
## 放射科醫師影像判讀重點
- **成因回到固定公式**：機器一律以 1540 m/s × 來回時間 ÷ 2 換算深度；判讀時先問聲束是否經過聲速偏離軟組織平均的介質（脂肪較慢、矽膠更慢），再判斷位移方向。[^1][^2]
- **方向判準：慢→深、快→淺**：實際聲速 < 1540（如脂肪 ~1450 m/s，慢約 6%）→ 回波晚到 → 標的被置於**更深**；> 1540 則置於更淺；皆為**軸向**位移，不會擺到外側。[^1][^2]
- **典型視覺表徵＝介面階梯狀錯位／斷裂**：經脂肪那段聲束下方結構整段往深方推移，使本應連續的界面出現局部不連續（focal discontinuity），肝—橫膈介面為經典例（focal steatosis 使橫膈遠端位移／看似截斷）。[^2][^3]
- **量測後果**：沿聲束的距離／深度量測會失準，脂肪或矽膠介入聲束路徑時尤甚；做精確測距時須意識此誤差，必要時避開脂肪路徑。[^2][^4]
- **判讀破解法：改角度／換切面重掃**——此偽影隨探頭幾何改變而出現或消失；懷疑「主動脈重複」「界面斷裂」等為偽影而非真病灶時，真結構在多平面一致、偽影隨幾何變動。[^3][^5]

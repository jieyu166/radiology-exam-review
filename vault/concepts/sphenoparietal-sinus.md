---
concepts: [sphenoparietal-sinus]
name: Sphenoparietal Sinus — Drainage Patterns
subspecialty: [NR]
aliases:
  - sphenoparietal sinus
  - sphenoparietal sinus of Breschet
  - 蝶頂竇
dateRev: 2026-06-15
---

# sphenoparietal-sinus

**蝶頂竇沿蝶骨小翼後下緣走行，收 superficial middle cerebral vein（Sylvian vein）後有三條出路；考題常問「哪個不是引流型」，陷阱是把屬支（MCV 流入竇）誤當出路（竇流向 SSS）。** 判讀分水嶺一：**superficial MCV 是流『入』蝶頂竇的屬支，不是竇流出的出路**——方向相反。判讀分水嶺二：三型出路＝①海綿竇（最常見、前內側）→②翼叢（sphenobasal，basal emissary）→③沿中顱窩底越岩錐入岩下/橫竇（sphenopetrosal）；空間記憶：前→海綿、下→翼叢、後→岩錐。

## Summary
- **解剖**：沿蝶骨小翼後下緣；接收 **superficial middle cerebral vein（Sylvian vein）**、middle meningeal vein（frontal ramus）、anterior temporal diploic vein；典型引流入**海綿竇**。[^1]
- **三種典型（深部）引流型**（依題目來源 Takahashi）：[^2]
  1. 入**海綿竇**（最常見、前內側）。
  2. 經 basal emissary veins 入**翼叢（pterygoid plexus）**（sphenobasal 型）。
  3. **沿中顱窩底向後、越岩錐**入**inferior petrosal vein/sinus 或 transverse sinus**（sphenopetrosal 型）。
- **非屬上述三型（題目 EXCEPT）**：「直接向上至 middle cerebral vein 再入上矢狀竇」——方向相反（MCV 是流『入』蝶頂竇的屬支），非其引流型。[^1][^2]

> [!note] 考點：除哪一項外？
> 三型＝海綿竇／翼叢（basal emissary）／沿中顱窩底越岩錐入岩下或橫竇。**「向上至 MCV 再入 SSS」非其引流型**（MCV 為其屬支、流向相反）。[^1][^2]

### 參考來源
[^1]: *Sphenoparietal sinus*. Radiopaedia.org。沿蝶骨小翼後下緣；接收 superficial middle cerebral vein 等屬支；引流入海綿竇（實際查證 accessed 2026-06-15）。
[^2]: Takahashi S. *Neurovascular Imaging*, p.264（題目所引；蝶頂竇三種典型引流型：海綿竇、翼叢、沿中顱窩底越岩錐入岩下/橫竇。**未逐字核實該書頁**，依題目陳述與 [^1] 解剖）。

## 題目
> [!question]- 蝶頂竇有三種典型引流型，下列除何者外？
> **「直接向上至 middle cerebral vein 再入上矢狀竇」**。三型為：海綿竇、翼叢（basal emissary）、沿中顱窩底越岩錐入岩下/橫竇。[^1][^2]

## 考題
```dataview
list from #交換 where contains(concepts, "sphenoparietal-sinus")
```

...
## 技術要點
- **PD 權重的參數組合**：**長 TR（減少 T1 對比）＋ 短 TE（減少 T2 對比）**，剩下的訊號差異即由質子密度主導。[^1]
- **常考陷阱**：「**短 TR ＋ 長 TE 產生 PD**」為錯誤敘述——該組合是 PD 的完全相反（短 TR 強調 T1、長 TE 強調 T2）。[^1]
- **SNR 特性**：長 TR 使縱向磁化恢復較完全，故 PD 之 SNR 高於相對應之 T1W 或 T2W。[^1]
- **MSK 主力應用**：對軟骨、半月板等組織之質子密度差異敏感，PD（常搭配 fat-sat）為關節成像主力序列。[^1]
- **後顱窩 MS 判讀優勢**：PD 上 CSF 不致過亮，後顱窩 MS 斑塊在 PD 可較 T2 更明顯、較不被高訊號 CSF 掩蓋。[^1]
- **記憶鉤**：兩參數皆「一長一短」——長 TR、短 TE，才能同時把 T1 與 T2 對比壓下、讓質子密度呈現。[^1]

> [!note] 考點：PD 影像何者「為非」?
> 「**短 TR ＋ 長 TE 產生 PD**」非——PD 為 **長 TR ＋ 短 TE**。PD SNR 高、MSK 敏感、後顱窩 MS 較 T2 明顯皆對。[^1]

### 參考來源
[^1]: 質子密度加權 MRI 標準物理：**長 TR（抑 T1）＋ 短 TE（抑 T2）→ PD 權重**;長 TR 致高 SNR;MSK（軟骨/半月板）主力;後顱窩 MS 斑塊可較 T2 明顯。**權威一手來源**：Pooley RA. *Fundamental Physics of MR Imaging*. RadioGraphics. 2005;25(4):1087-1099. DOI 10.1148/rg.254055027（RSNA，accessed 2026-07-05）——短 TE（最小化 T2 對比）＋ 長 TR（最小化 T1 對比）產生質子密度加權影像。

...
name: Calcifying Pulmonary Metastases
subspecialty: [CH]
aliases:
...
> [!note] 機轉口訣
> 成骨（osteosarcoma/chondrosarcoma）、軟骨／壞死（synovial sarcoma、GCT、papillary thyroid）、黏液（mucinous colon/ovary/breast）、甲狀腺（papillary/medullary）、治療後（chemo/RT）。

### 二、鈣化型態（pattern of calcification）：良性 vs 不確定／惡性
鈣化「型態」比「有無鈣化」更具鑑別力。CT 上 attenuation ≥ **200 HU** 一般即視為鈣化證據；HRCT 偵測細小鈣化較胸部 X 光敏感得多。[^1]

- **良性型態（4 種典型）**[^1][^4]
  - **central（中央緻密核心）**——肉芽腫（最常見 TB、histoplasmosis），部分 hamartoma。
  - **laminated／concentric（同心層狀）**——肉芽腫，幾乎可確診。
  - **diffuse（瀰漫實心）**——肉芽腫；hamartoma 亦可。
  - **popcorn（爆米花狀）**——**hamartoma（錯構瘤）幾乎可確診**（含軟骨成分鈣化）。
- **不確定／須警覺型態**[^1][^4]
  - **eccentric（偏心）、stippled／punctate（點狀）、amorphous（無定形）**——**不可視為良性**。
  - 高達約 **10% bronchogenic carcinoma** 可出現營養不良性鈣化（常為點狀／偏心）。
  - **陷阱**：惡性病灶吞噬（engulf）原已鈣化的良性肉芽腫，可造成「偏心鈣化」假象；細小結節偏心鈣化不易判定，易誤判為良性。[^1][^4]
...
| 塵肺／纖維化 | silicosis、coal worker's pneumoconiosis、progressive massive fibrosis | 上肺為主、可蛋殼狀淋巴結鈣化 [^1][^4] |

**瀰漫性多發小鈣化結節**之鑑別：感染、肺轉移、慢性肺出血、塵肺、沉積病、以及 idiopathic 的 **pulmonary alveolar microlithiasis（肺泡微石症）**。[^1]

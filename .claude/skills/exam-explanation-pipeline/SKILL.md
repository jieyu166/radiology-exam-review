...
description: "為交換考題逐批製作逐選項詳解、補放射來源 reference、補分類，並自評稽核。以官方詳解 PDF 為事實基礎、vault 筆記交叉補強，產出 rex-edits patch 供醫師覆核。在 Claude Code session 內依當時 token 剩餘量逐批執行，年份優先 2016→2024。Trigger：製作詳解、補 reference、補分類、跑詳解流水線、explanation pipeline。"
effort: high
license: MIT
metadata:
  author: jieyu166

...
def test_validator_accepts_footnote_metadata_under_reference_subsection() -> None:
    text = (
        "---\nconcepts: [demo]\nsubspecialty: [NR]\n---\n"
        "## Summary\n"
        "- **Label**: Supported fact.[^1]\n"
        "\n"
        "### 參考來源\n"
        "[^1]: Complete article citation.\n"
        "    Continuation with DOI 10.1000/example.\n"
        "\n## Body\nBody.\n"
    )
    findings = audit.validate_summary(audit.parse_note_text(Path("demo.md"), text))
    assert findings == []
...
BATCH03_PREEDIT_SHA256 = {
    "brain-tumor-imaging": (
        "1469181203fdd066b27fea6083e08016e080eda89dacb5a7b01a17a3868a6712"
    ),
    "cerebral-infarction-fogging": (
        "73e2d83c0192f53329f0d8566a3b1c708e1b8e27b2044e7cab66d2dc37e3c26b"
    ),
    "cerebral-microbleeds": (
        "c4333658d946d97f65f0dc4661fd9705517f0742dfd64ec296f3f2947da3b954"
    ),
    "cerebrovascular-malformations": (
        "5a19d890855886093f4525cc9fff0a9748b7d92faa27947a151d1d7ef9d0f280"
    ),
    "chemical-shift-artifact": (
        "31ebc729f179b3ad4191edf6591f4c305546a205be550413335cf96bcfcdc3f4"
    ),
    "cns-opportunistic-infection": (
        "fa995cb436473c80d77be3b9d031d2b497e16c66bbe94488c15707dfd29a7c23"
    ),
    "cranial-nerve-muscle-atrophy": (
        "d8a236e3502cdae423a8e021b4271b857b648b35d4b34c3bc72c1d0bd3a5bc0c"
    ),
    "dural-based-masses-aids": (
        "43d0d0133c7103d0f497fbfead8627769727c6a51e28a2e2514307198a5b6399"
    ),
    "facial-fracture-complications": (
        "f924b9aa549bbaa37d3166d0980c0012789eda33a0b7788a7cc4b196373c5c16"
    ),
    "gbm-vs-pcnsl": (
        "ddb316644e5746ac0f6f7616605028ba8d4d772da152c3c999f3fc56ecfe48d9"
    ),
}
BATCH03_STATEMENT_FACT_EMPTY_REF_COUNTS = {
    "brain-tumor-imaging": (5, 7, 0),
    "cerebral-infarction-fogging": (5, 5, 0),
    "cerebral-microbleeds": (8, 11, 0),
    "cerebrovascular-malformations": (0, 0, 0),
    "chemical-shift-artifact": (19, 34, 0),
    "cns-opportunistic-infection": (5, 7, 1),
    "cranial-nerve-muscle-atrophy": (4, 6, 0),
    "dural-based-masses-aids": (6, 7, 3),
    "facial-fracture-complications": (14, 14, 0),
    "gbm-vs-pcnsl": (3, 3, 0),
}
...
def _assert_only_unsealed_batch03_chain(findings: list[audit.Finding]) -> None:
    """Task 4.2 leaves exactly the later Batch 03 generated seal incomplete."""
    assert [
...
def _production_phase2a_batch03() -> tuple[Path, Path, audit.BatchContext]:
    root = Path(__file__).resolve().parents[1]
    assignment_path = Path(
        "docs/reports/nr-summary-rewrite/phase2-assignment.json"
    )
...
def test_phase2a_batch03_builder_accepts_only_the_exact_empty_summary_projection() -> None:
    with tempfile.TemporaryDirectory() as directory:
        context = _load_preedit_phase2a_batch03_shadow(Path(directory))
        templates = {
            slug: audit.phase2_default_fact_templates(note)
            for slug, note in context.note_records.items()
        }
        built = audit.build_phase2_baseline_lock(context, templates)
        empty = next(
            entry
            for entry in built["notes"]
            if entry["slug"] == "cerebrovascular-malformations"
        )
        assert empty["summaryHeadings"] == []
        assert empty["originalSummary"] == ""
        assert empty["factUnits"] == []

        forged_context = replace(
            context,
            note_records={
                **context.note_records,
                "cerebrovascular-malformations": audit.parse_note_text(
                    Path("vault/concepts/cerebrovascular-malformations.md"),
                    (
                        "---\nsubspecialty: [NR]\n---\n"
                        "## Summary\n"
                        "### Heading without factual content\n"
                    ),
                ),
            },
        )
        forged_templates = {
            slug: audit.phase2_default_fact_templates(note)
            for slug, note in forged_context.note_records.items()
        }
        try:
            audit.build_phase2_baseline_lock(forged_context, forged_templates)
        except ValueError:
            pass
        else:
            raise AssertionError(
                "An accepted Summary may not hide behind an empty fact lock."
            )


def _batch03_resealed_codes(baseline: dict) -> set[str]:
    root, _, context = _production_phase2a_batch03()
    batch01 = json.loads(
        (
            root
    ...
    return _validate_resealed_baseline(
        ...
        registry={
            ...
            "batch-02-disease": canonical_sha256(batch02),
            "batch-03-pattern": canonical_sha256(baseline),
        },
    )


def _copy_production_phase2a_batch03_checkout(destination: Path) -> Path:
    root, assignment_path, batch03 = _production_phase2a_batch03()
    relative_files = [
        assignment_path,
        batch03.inventory_path,
        batch03.baseline_path.parent / "batch-01-anatomy.json",
        batch03.baseline_path.parent / "batch-02-disease.json",
        batch03.baseline_path,
        batch03.evidence_path,
        *(Path(record.path) for record in batch03.note_records.values()),
    ]
...
def _restore_preedit_phase2a_batch03_sources(
    destination: Path, assignment_path: Path
) -> audit.BatchContext:
    """Restore Batch 03 sources strictly from the authenticated baseline spans."""
    current = audit.load_phase2_batch(
        destination, assignment_path, "batch-03-pattern"
    )
    assert isinstance(current.baseline, dict)
    for locked in current.baseline["notes"]:
        slug = locked["slug"]
        note = current.note_records[slug]
        source_path = destination / note.path
        source_text = source_path.read_bytes().decode("utf-8")
        assert note.original_summary
        assert source_text.count(note.original_summary) == 1
        replacement = (
            ""
            if audit._phase2_is_approved_empty_projection_migration(
                current, locked
            )
            else locked["originalSummary"]
        )
        source_path.write_bytes(
            source_text.replace(note.original_summary, replacement, 1).encode(
                "utf-8"
            )
        )
...
def _load_preedit_phase2a_batch03_shadow(destination: Path) -> audit.BatchContext:
    """Reconstruct Task 4.1 source/evidence state without trusting current summaries."""
    assignment_path = _copy_production_phase2a_batch03_checkout(destination)
    restored = _restore_preedit_phase2a_batch03_sources(
        destination, assignment_path
    )
    pending_bytes = audit.build_phase2_pending_evidence_scaffold_bytes(
        restored,
        implementer="/root/phase2a_task4_1_impl",
    )
    (destination / restored.evidence_path).write_bytes(pending_bytes)
    return audit.load_phase2_batch(
        destination, assignment_path, "batch-03-pattern"
    )


def test_phase2a_batch03_baseline_is_exact_lossless_and_byte_deterministic() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        context = _load_preedit_phase2a_batch03_shadow(root)
        baseline = context.baseline
        assert isinstance(baseline, dict)
        assert context.batch["type"] == "pattern-ddx"
        assert context.batch["slugs"] == list(BATCH03_PATTERN_SLUGS)
        ...
        inventory = json.loads(
            (root / context.inventory_path).read_text(encoding="utf-8")
        )
        inventory_by_slug = {entry["slug"]: entry for entry in inventory["notes"]}
        for entry in baseline["notes"]:
            slug = entry["slug"]
            ...
            assert entry["type"] == "pattern-ddx"
            assert entry["originalSha256"] == BATCH03_PREEDIT_SHA256[slug]
            assert entry["originalSha256"] == note.sha256
            assert entry["summaryHeadings"] == inventory_by_slug[slug][
                "summaryHeadings"
            ]
        ...
        expected_bytes = audit.build_phase2_baseline_lock_bytes(
            context,
            {
                slug: audit.phase2_default_fact_templates(note)
                for slug, note in context.note_records.items()
        ...
        assert expected_bytes == baseline_path.read_bytes()
        assert (
            canonical_sha256(baseline)
            == audit.TRUSTED_PHASE2A_BATCH_LOCK_SHA256["batch-03-pattern"]
        )


def test_phase2a_batch03_fact_projection_is_complete_stable_and_source_exact() -> None:
    with tempfile.TemporaryDirectory() as directory:
        context = _load_preedit_phase2a_batch03_shadow(Path(directory))
        baseline = context.baseline
        assert isinstance(baseline, dict)
        ...
        for entry in baseline["notes"]:
            ...
            observed[slug] = (
                len(statements),
                len(facts),
                sum(not fact["sourceRefs"] for fact in facts),
            )
            ...
            assert all(
                not audit.FOOTNOTE_DEFINITION_RE.match(fact["sourceStatement"])
                for fact in facts
            )
            assert all(
        ...
        assert observed == BATCH03_STATEMENT_FACT_EMPTY_REF_COUNTS
        assert sum(value[0] for value in observed.values()) == 69
        assert sum(value[1] for value in observed.values()) == 94


def test_phase2a_batch03_preserves_nested_context_callouts_and_empty_refs() -> None:
    _, _, context = _production_phase2a_batch03()
    baseline = context.baseline
    assert isinstance(baseline, dict)
    by_slug = {entry["slug"]: entry for entry in baseline["notes"]}

    micro = by_slug["cerebral-microbleeds"]["factUnits"]
    micro_parent = "- **常見病因（分佈導向）**：[^1][^2][^3]"
    assert sum(fact["sourceStatement"] == micro_parent for fact in micro) == 1
    assert next(
        fact for fact in micro if fact["sourceStatement"] == micro_parent
    )["sourceRefs"] == ["1", "2", "3"]

    dural = by_slug["dural-based-masses-aids"]["factUnits"]
    dural_parent = (
        "- **腦膜瘤(meningioma)之其他鑑別"
        "（Mass Lesions of the Dura綜述）**："
    )
    assert sum(fact["sourceStatement"] == dural_parent for fact in dural) == 1
    assert next(
        fact for fact in dural if fact["sourceStatement"] == dural_parent
    )["sourceRefs"] == ["1"]

    facial = by_slug["facial-fracture-complications"]["factUnits"]
    assert [
        fact["id"]
        for fact in facial
        if fact["text"].endswith("：")
    ] == [
        "facial-fracture-complications-f01",
        "facial-fracture-complications-f05",
        "facial-fracture-complications-f09",
        "facial-fracture-complications-f12",
    ]
    assert all(fact["sourceRefs"] == ["1"] for fact in facial)

    brain = by_slug["brain-tumor-imaging"]["factUnits"]
    assert sum(
        fact["sourceStatement"].startswith("> 「**Ganglioglioma")
        for fact in brain
    ) == 2
    chemical = by_slug["chemical-shift-artifact"]["factUnits"]
    assert sum(
        fact["sourceStatement"].startswith("> 「**純脂肪區")
        for fact in chemical
    ) == 3
    ...
    empty_note = by_slug["cerebrovascular-malformations"]
    assert empty_note["summaryHeadings"] == []
    assert empty_note["originalSummary"] == ""
    assert empty_note["factUnits"] == []


def test_phase2a_batch03_rejects_empty_fact_and_membership_order_attacks() -> None:
    _, _, context = _production_phase2a_batch03()
    baseline = context.baseline
    assert isinstance(baseline, dict)

    accepted_empty_facts = deepcopy(baseline)
    accepted_empty_facts["notes"][0]["factUnits"] = []
    assert "phase2-baseline-schema" in _batch03_resealed_codes(
        accepted_empty_facts
    )

    hidden_accepted_span = deepcopy(baseline)
    hidden_accepted_span["notes"][0]["summaryHeadings"] = []
    hidden_accepted_span["notes"][0]["originalSummary"] = ""
    hidden_accepted_span["notes"][0]["factUnits"] = []
    assert "phase2-baseline-inventory-mismatch" in _batch03_resealed_codes(
        hidden_accepted_span
    )

    note_mutations = []
    missing = deepcopy(baseline)
    missing["notes"].pop()
    note_mutations.append(missing)
    duplicate = deepcopy(baseline)
    duplicate["notes"].append(deepcopy(duplicate["notes"][0]))
    note_mutations.append(duplicate)
    extra = deepcopy(baseline)
    extra_entry = deepcopy(extra["notes"][0])
    extra_entry["slug"] = "not-in-batch-03"
    extra["notes"].append(extra_entry)
    note_mutations.append(extra)
    reordered = deepcopy(baseline)
    reordered["notes"][0], reordered["notes"][1] = (
        reordered["notes"][1],
        reordered["notes"][0],
    )
    note_mutations.append(reordered)
    for changed in note_mutations:
        assert "phase2-baseline-inventory-mismatch" in _batch03_resealed_codes(
            changed
        )

    duplicate_fact = deepcopy(baseline)
    duplicate_fact["notes"][0]["factUnits"][1]["id"] = (
        duplicate_fact["notes"][0]["factUnits"][0]["id"]
    )
    reordered_facts = deepcopy(baseline)
    reordered_facts["notes"][0]["factUnits"][0:2] = reversed(
        reordered_facts["notes"][0]["factUnits"][0:2]
    )
    malformed_ref = deepcopy(baseline)
    malformed_ref["notes"][0]["factUnits"][0]["sourceRefs"] = ["undefined"]
    for changed in (duplicate_fact, reordered_facts, malformed_ref):
        assert "phase2-baseline-schema" in _batch03_resealed_codes(changed)


def test_phase2a_batch03_registry_is_exact_three_batch_prefix() -> None:
    root, _, context = _production_phase2a_batch03()
    baseline = context.baseline
    assert isinstance(baseline, dict)
    expected = {
    ...
    assert audit.TRUSTED_PHASE2A_BATCH_LOCK_SHA256 == expected
    for registry in (
        {},
        {"batch-01-anatomy": expected["batch-01-anatomy"]},
        {
            "batch-01-anatomy": expected["batch-01-anatomy"],
            "batch-03-pattern": expected["batch-03-pattern"],
        },
        {
            **expected,
            "batch-03-pattern": "0" * 64,
        },
        {
            **expected,
            "unknown-batch": "1" * 64,
...
def test_phase2a_batch03_pending_scaffold_is_honest_and_nonterminal() -> None:
    with tempfile.TemporaryDirectory() as directory:
        context = _load_preedit_phase2a_batch03_shadow(Path(directory))
        ...
        baseline_by_slug = {
            entry["slug"]: entry for entry in context.baseline["notes"]
        }
        for entry in evidence["notes"]:
            locked = baseline_by_slug[entry["slug"]]
            assert entry["status"] == "pending"
            assert entry["rewrittenSummary"] == locked["originalSummary"]
            assert entry["facts"] == [
                {
                    "id": fact["id"],
                    "sourceRefs": fact["sourceRefs"],
            ...
            assert all(
                value["kind"] == "existing-footnote"
                for value in entry["sourceDefinitions"].values()
            )
            assert all(
                key not in entry
                for key in (
        ...
        assert "phase2-evidence-schema" in final_codes
        assert "evidence-fact-coverage" in final_codes
        assert "phase2-reviewer-conflict" in final_codes


def test_phase2a_batch03_relocated_checkout_has_identical_baseline_bytes() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory) / "canonical"
        context = _load_preedit_phase2a_batch03_shadow(root)
        canonical_bytes = (root / context.baseline_path).read_bytes()
        canonical_codes = [
            finding.code
            for finding in (
        ...
        relocated_root = Path(directory) / "relocated" / "checkout"
        relocated = _load_preedit_phase2a_batch03_shadow(
            relocated_root
        )
        regenerated = audit.build_phase2_baseline_lock_bytes(
            relocated,
            {
                slug: audit.phase2_default_fact_templates(note)
                for slug, note in relocated.note_records.items()
            },
        )
        relocated_codes = [
            finding.code
            for finding in (
                audit.validate_baseline_lock(relocated)
                + audit._validate_phase2_source_state(
                    relocated, pre_edit=True
                )
            )
        ]
    assert canonical_codes == relocated_codes == []
    assert regenerated == canonical_bytes


def test_phase2a_batch03_coordinated_mutable_replacement_is_untrusted() -> None:
    root, assignment_path, context = _production_phase2a_batch03()
    with tempfile.TemporaryDirectory() as directory:
        shadow = Path(directory) / "coordinated"
        _copy_production_phase2a_batch03_checkout(shadow)
        slug = context.batch["slugs"][0]
        source_path = shadow / context.note_records[slug].path
        source_path.write_bytes(source_path.read_bytes() + b"\n")
        replacement_hash = hashlib.sha256(source_path.read_bytes()).hexdigest()

        inventory_path = shadow / context.inventory_path
        inventory = json.loads(inventory_path.read_text(encoding="utf-8"))
        next(
            entry for entry in inventory["notes"] if entry["slug"] == slug
        )["originalSha256"] = replacement_hash
        assignment = audit.build_phase2_assignment(inventory)
        inventory = audit.synchronize_phase2_inventory(inventory, assignment)
        inventory_path.write_text(json.dumps(inventory), encoding="utf-8")
        ...
        next(
            entry for entry in baseline["notes"] if entry["slug"] == slug
        )["originalSha256"] = replacement_hash
        baseline_path.write_text(json.dumps(baseline), encoding="utf-8")
        ...
        attacked = audit.load_phase2_batch(
            shadow, assignment_path, "batch-03-pattern"
        )
        codes = {
            finding.code
            for finding in audit.validate_baseline_lock(attacked)
        }
    assert codes == {"phase2-trusted-batch-lock-mismatch"}


def test_phase2a_batch03_preserves_both_approved_predecessors() -> None:
    root, _, _ = _production_phase2a_batch03()
    for expected_map in (
        BATCH01_APPROVED_ARTIFACT_SHA256,
        BATCH02_APPROVED_ARTIFACT_SHA256,
    ):
    ...
    for batch_id in ("batch-01-anatomy", "batch-02-disease"):
        context = audit.load_phase2_batch(
            root,
...
def test_phase2a_batch03_workflow_rejects_predecessor_loss_before_write() -> None:
    root, assignment_path, context = _production_phase2a_batch03()
    original_validate = audit.validate_phase2_batch
    original_generated_registry = (
        audit.TRUSTED_PHASE2A_GENERATED_OBSERVATION_SHA256
    )
    try:
        for mutation in ("missing-baseline", "wrong-generated-seal"):
            with tempfile.TemporaryDirectory() as directory:
                ...
                before = audit._phase2_generated_snapshot(shadow_context)
                audit.validate_phase2_batch = lambda *args, **kwargs: []
                with _expect_phase2_error() as observed:
                    audit.run_phase2_generated_observation_workflow(
                        shadow, "batch-03-pattern"
                    )
                assert observed["code"] == "phase2-review-sequence"
                assert audit._phase2_generated_snapshot(shadow_context) == before
                audit.validate_phase2_batch = original_validate
                audit.TRUSTED_PHASE2A_GENERATED_OBSERVATION_SHA256 = (
                    original_generated_registry
                )
    finally:
...
def _copy_production_phase2a_batch02_checkout(destination: Path) -> Path:
    """Copy both reviewed batches and the complete generated corpus."""
    root, assignment_path, batch02 = _production_phase2a_batch02()
    _, _, batch01 = _production_phase2a_batch01()
    relative_files = [
        assignment_path,
        batch02.inventory_path,
        batch01.baseline_path,
        batch01.evidence_path,
        batch02.baseline_path,
        batch02.evidence_path,
        batch02.baseline_path.parent / "batch-03-pattern.json",
        Path("data/concepts-index.json"),
        *(
            Path(record.path)
            for context in (batch01, batch02)
            for record in context.note_records.values()
        ),
    ]
    for batch_id in ("batch-01-anatomy", "batch-02-disease"):
...
def test_phase2a_batch01_summaries_are_flat_sourced_cards_with_stable_facts() -> None:
    ...
    for locked in baseline["notes"]:
        ...
        lines = [
            ...
            if audit.TOP_LEVEL_BULLET_RE.match(line)
        ]
        assert lines
...
def test_phase2a_batch01_production_generated_seal_is_genuine_and_relocation_stable() -> None:
    ...
    assert set(audit.TRUSTED_PHASE2A_GENERATED_OBSERVATION_SHA256) == {
        "batch-01-anatomy",
        "batch-02-disease",
        "batch-03-pattern",
    }
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        shadow_assignment = _copy_production_phase2a_task43_checkout(shadow)
        ...
        shadow_findings = audit.validate_phase2_batch(
            shadow_context,
            check_source_hashes=False,
            check_generated=True,
        )
...
def test_phase2a_batch01_generated_registry_rejects_missing_wrong_and_extra_trust() -> None:
    ...
    registries = (
        ...
        ({"batch-01-anatomy": "0" * 64}, "generated-observation-untrusted"),
        (
            {
    ...
    try:
        for registry, expected_code in registries:
            ...
            assert expected_code in codes, registry
    finally:
...
def _batch02_resealed_codes(baseline: dict) -> set[str]:
    ...
    return _validate_resealed_baseline(
        ...
        registry={
            ...
            "batch-03-pattern": canonical_sha256(batch03),
        },
    )
...
def test_phase2a_batch02_baseline_is_exact_lossless_and_byte_deterministic() -> None:
    ...
    assert baseline["assignmentSha256"] == canonical_sha256(context.assignment)
    assert audit.validate_baseline_lock(context) == []
    assert audit._validate_phase2_source_state(context, pre_edit=False) == []
    ...
    for entry in baseline["notes"]:
        ...
        assert entry["originalSummary"] != note.original_summary
        assert (
            audit._phase2_reconstructed_original_sha256(
                context,
                entry,
                note,
            )
    ...
    assert (
        canonical_sha256(baseline)
        == audit.TRUSTED_PHASE2A_BATCH_LOCK_SHA256["batch-02-disease"]
    )
def test_phase2a_batch02_fact_units_cover_every_source_statement_in_order() -> None:
    ...
    for entry in baseline["notes"]:
        ...
        locked_note = audit.parse_note_text(
            ...
            (
                "---\nsubspecialty: [NR]\n---\n"
                f"{entry['originalSummary']}"
            ),
        )
        facts = entry["factUnits"]
        statements = audit.phase2_summary_source_statements(locked_note)
        referenced = list(
            dict.fromkeys(
                ref
                for fact in facts
        ...
        assert [
            ...
            for fact in facts
        ] == audit.phase2_default_fact_templates(locked_note)
        assert all(fact["text"].strip() for fact in facts)
        assert all(
            isinstance(fact["sourceRefs"], list)
            and len(fact["sourceRefs"]) == len(set(fact["sourceRefs"]))
...
def test_phase2a_batch02_registry_is_exact_contiguous_prefix_and_fail_closed() -> None:
    ...
    expected_registry = {
        "batch-01-anatomy": canonical_sha256(batch01),
        "batch-02-disease": canonical_sha256(baseline),
        "batch-03-pattern": canonical_sha256(
            json.loads(
                (
                    root
                    / "docs"
...
def test_phase2a_batch02_pending_scaffold_builder_remains_honest() -> None:
    _, _, context = _production_phase2a_batch02()
    baseline = context.baseline
    assert isinstance(baseline, dict)
    pending = audit.build_phase2_pending_evidence_scaffold(
        context,
        implementer="/root/phase2a_task3_1_impl",
    )
    assert pending["status"] == "baseline"
    ...
    assert pending["manualReviewFactIds"] == []
    assert pending["generatedManifest"] == (
        "docs/reports/nr-summary-rewrite/phase2a/generated/"
        "batch-02-disease.json"
    )
    assert [entry["slug"] for entry in pending["notes"]] == list(
        BATCH02_DISEASE_SLUGS
    )
    assert all(entry["status"] == "pending" for entry in pending["notes"])
    assert all(
        fact["disposition"] == "pending"
        for entry in pending["notes"]
        for fact in entry["facts"]
    )
    assert all(
        "coverageEvidenceSha256" not in entry
...
def test_phase2a_batch02_baseline_has_relocated_checkout_parity() -> None:
    ...
    canonical_codes = [
        ...
        for finding in (
            audit.validate_baseline_lock(context)
            + audit._validate_phase2_source_state(context, pre_edit=False)
        )
    ]
    with tempfile.TemporaryDirectory() as directory:
        shadow = Path(directory) / "relocated" / "shadow"
        paths = [
            ...
            context.baseline_path.parent / "batch-01-anatomy.json",
            context.baseline_path,
            context.baseline_path.parent / "batch-03-pattern.json",
            context.evidence_path,
            *(Path(record.path) for record in context.note_records.values()),
        ]
        for relative in paths:
...
def test_phase2a_batch02_preserves_approved_batch01_prerequisite() -> None:
    ...
    assert audit.validate_baseline_lock(batch02_context) == []
...
def test_phase2a_batch02_rewrite_preserves_every_non_summary_byte() -> None:
    root, _, context = _production_phase2a_batch02()
    baseline = context.baseline
    assert isinstance(baseline, dict)
...
def test_phase2a_batch02_rewrite_evidence_is_deterministic_and_reviewed() -> None:
    root, _, context = _production_phase2a_batch02()
    expected_queue = [
        "adrenoleukodystrophy-f30",
        "adrenoleukodystrophy-f33",
    ...
    reviewed = deepcopy(regenerated)
    reviewed["workflow"]["reviewStatus"] = "approved"
    reviewed["workflow"]["reviewedBaselineSha256"] = canonical_sha256(
        context.baseline
    )
    assert reviewed == context.evidence
    assert (
    ...
    assert evidence["workflow"] == {
        ...
        "reviewer": "/root/phase2a_task3_3_review",
        "reviewStatus": "approved",
        "reviewedBaselineSha256": canonical_sha256(context.baseline),
    }
    facts = [fact for entry in evidence["notes"] for fact in entry["facts"]]
    assert len(facts) == 98
...
def test_phase2a_batch02_summaries_are_sourced_disease_cards() -> None:
    ...
    aicardi = context.note_records["aicardi-syndrome"].original_summary
    assert aicardi.count("典型三主徵") == 1
    assert all(
        marker in aicardi
        for marker in (
            "Corpus callosal dysgenesis/agenesis",
    ...
    unresolved = {
        fact["id"]: fact
        for fact in evidence_by_slug["adrenoleukodystrophy"]["facts"]
        if fact["disposition"] != "covered"
    }
    assert unresolved == {
        "adrenoleukodystrophy-f30": {
...
def test_phase2a_batch02_reviewed_generated_seal_and_relocation_are_genuine() -> None:
    root, assignment_path, context = _production_phase2a_batch02()
    evidence = context.evidence
    assert isinstance(evidence, dict)
    baseline_digest = canonical_sha256(context.baseline)
    ...
    assert audit.TRUSTED_PHASE2A_GENERATED_OBSERVATION_SHA256 == {
        ...
        "batch-02-disease": observation_digest,
        "batch-03-pattern": canonical_sha256(
            audit._phase2_generated_observation_projection(batch03_manifest)
        ),
    }
    assert checked["selectedSlugs"] == list(BATCH02_DISEASE_SLUGS)
    assert len(checked["detailFiles"]) == 10
    assert checked["detailFileCount"] == 980
    assert checked["index"]["entryCount"] == 980
    assert checked["secondRun"] == {
        "changedPaths": [],
        "mtimeChangedPaths": [],
    }
    for batch_id in ("batch-01-anatomy", "batch-02-disease"):
        batch_context = audit.load_phase2_batch(root, assignment_path, batch_id)
        assert audit.validate_phase2_batch(
            batch_context,
            check_source_hashes=False,
            check_generated=True,
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        shadow_assignment = _copy_production_phase2a_task43_checkout(shadow)
        shadow_context = audit.load_phase2_batch(
            shadow, shadow_assignment, "batch-02-disease"
        )
        own_findings, own_state = audit._validate_phase2_own_generated_result(
            shadow_context
        )
        assert own_findings == []
        assert own_state is not None
        assert own_state.checked == checked
        assert canonical_sha256(
            audit._phase2_generated_observation_projection(own_state.checked)
        ) == observation_digest
...
def test_phase2a_batch02_workflow_rejects_review_and_predecessor_forgery_before_write() -> None:
    cases = (
        ("unapproved", "phase2-review-sequence"),
        ("same-identity", "phase2-reviewer-conflict"),
        ("wrong-baseline", "phase2-review-sequence"),
        ("unresolved-queue", "phase2-manual-queue-mismatch"),
    )
    for mutation, expected_code in cases:
        with tempfile.TemporaryDirectory() as directory:
            shadow = Path(directory) / mutation
            assignment_path = _copy_production_phase2a_batch02_checkout(shadow)
            evidence_path = (
                shadow
                / "docs/reports/nr-summary-rewrite/phase2a/evidence/"
                "batch-02-disease.json"
            ...
            if mutation == "unapproved":
                evidence["workflow"]["reviewStatus"] = "not-started"
                evidence["workflow"]["reviewedBaselineSha256"] = None
            elif mutation == "same-identity":
                evidence["workflow"]["reviewer"] = evidence["workflow"][
                    "implementer"
                ]
            elif mutation == "wrong-baseline":
                evidence["workflow"]["reviewedBaselineSha256"] = "0" * 64
            else:
            ...
            context = audit.load_phase2_batch(
                shadow, assignment_path, "batch-02-disease"
            )
            before = audit._phase2_generated_snapshot(context)
            with _expect_phase2_error() as observed:
                audit.run_phase2_generated_observation_workflow(
                    shadow, "batch-02-disease"
                )
            assert observed["code"] == expected_code, mutation
            assert audit._phase2_generated_snapshot(context) == before
    ...
    try:
        for registry in (
            {},
            {"batch-01-anatomy": "0" * 64},
            {"batch-02-disease": original_registry.get("batch-02-disease", "")},
        ):
            with tempfile.TemporaryDirectory() as directory:
                shadow = Path(directory) / "predecessor"
                assignment_path = _copy_production_phase2a_batch02_checkout(shadow)
                context = audit.load_phase2_batch(
                    shadow, assignment_path, "batch-02-disease"
                )
...
def test_phase2a_batch02_workflow_accepts_exact_predecessor_before_own_seal() -> None:
    root, assignment_path, batch02_context = _production_phase2a_batch02()
    batch01_context = audit.load_phase2_batch(
        root, assignment_path, "batch-01-anatomy"
    )
    own_findings, predecessor_state = audit._validate_phase2_own_generated_result(
        batch01_context
    )
    assert own_findings == []
    ...
    batch03_paths = {
        (
            batch02_context.generated_root / f"{slug}.json"
        ).as_posix()
        for slug in audit.ACTIVE_PHASE2A_BATCHES["batch-03-pattern"]["slugs"]
    }
    historical_current_hashes = dict(predecessor_state.current_hashes)
    for path in batch03_paths:
        historical_current_hashes[path] = predecessor_state.historical_hashes[
            path
        ]
    historical_predecessor_state = replace(
        predecessor_state,
        current_hashes=historical_current_hashes,
    )
    batch02_selected_paths = {
        (
            batch02_context.generated_root / f"{slug}.json"
        ).as_posix()
        for slug in batch02_context.batch["slugs"]
    }
    assert audit._phase2_historical_authorization_findings(
        historical_predecessor_state,
        batch02_selected_paths,
    ) == []

    with tempfile.TemporaryDirectory() as directory:
        shadow = Path(directory) / "pre-own-seal"
        shadow_assignment = _copy_production_phase2a_batch02_checkout(shadow)
        shadow_context = audit.load_phase2_batch(
            shadow, shadow_assignment, "batch-02-disease"
        )
        before = audit._phase2_generated_snapshot(shadow_context)
...
def test_phase2a_batch02_generated_registry_and_manual_queue_are_fail_closed() -> None:
    _, _, context = _production_phase2a_batch02()
    evidence = context.evidence
    assert isinstance(evidence, dict)
    facts = [fact for entry in evidence["notes"] for fact in entry["facts"]]
    unresolved = [fact for fact in facts if fact["disposition"] != "covered"]
    assert evidence["status"] == "needs-review"
    assert evidence["manualReviewFactIds"] == [
        "adrenoleukodystrophy-f30",
        "adrenoleukodystrophy-f33",
    ...
    original_registry = audit.TRUSTED_PHASE2A_GENERATED_OBSERVATION_SHA256
    valid01 = original_registry.get("batch-01-anatomy")
    valid02 = original_registry.get("batch-02-disease")
    registries = (
        ({}, "generated-observation-untrusted"),
        (
            {"batch-01-anatomy": valid01},
            "generated-observation-untrusted",
        ),
        (
            {"batch-01-anatomy": valid01, "batch-02-disease": "0" * 64},
            "generated-observation-untrusted",
        ),
        (
            {"batch-02-disease": valid02},
            "generated-observation-untrusted",
        ),
        (
            {
    ...
    forged["manualReviewFactIds"] = []
    ald = next(
        entry for entry in forged["notes"] if entry["slug"] == "adrenoleukodystrophy"
    )
    ald["status"] = "verified"
    codes = {
        finding.code
...
def test_phase2a_batch02_selected_and_coordinated_generated_mutations_fail() -> None:
    with tempfile.TemporaryDirectory() as directory:
        shadow = Path(directory) / "selected"
        assignment_path = _copy_production_phase2a_batch02_checkout(shadow)
        context = audit.load_phase2_batch(
            shadow, assignment_path, "batch-02-disease"
        )
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
        entry = next(item for item in evidence["notes"] if item["slug"] == slug)
        entry["rewrittenSummary"] = mutated_note.original_summary
        entry["summaryBulletEvidence"] = audit._generated_keypoints(mutated_note)
        entry["coverageEvidenceSha256"] = audit.phase2_coverage_evidence_sha256(
            evidence["baselineLock"]["sha256"], entry
        )
        evidence_path.write_text(
        ...
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        relative = (context.generated_root / f"{slug}.json").as_posix()
        manifest["detailFiles"][relative] = hashlib.sha256(
            detail_path.read_bytes()
        ).hexdigest()
        historical = dict(manifest["nonselectedAfter"])
        historical.update(manifest["detailFiles"])
        manifest["detailTreeSha256"] = canonical_sha256(
            [
                {"path": path, "sha256": historical[path]}
...
def test_phase2a_batch02_workflow_rejects_unrelated_and_second_run_drift() -> None:
    import build_concepts as concept_builder

    original_builder = concept_builder.build_selected_concepts
    for phase, mutation, expected in (
        ("first", "bytes", "generated-unrelated-write"),
        ("first", "mtime", "generated-unrelated-write"),
        ("second", "bytes", "generated-non-idempotent"),
        ("second", "mtime", "generated-non-idempotent"),
    ):
        with tempfile.TemporaryDirectory() as directory:
            shadow = Path(directory) / f"{phase}-{mutation}"
            assignment_path = _copy_production_phase2a_batch01_checkout(shadow)
            context = audit.load_phase2_batch(
                shadow, assignment_path, "batch-01-anatomy"
            )
            ...
            victim = (
                next(
                    path
                    for path in sorted(
                        (shadow / context.generated_root).glob("*.json")
                    )
                    if path.stem not in selected
                )
                if phase == "first"
            ...
            def mutating_builder(*args, **kwargs):
                ...
                result = original_builder(*args, **kwargs)
                calls += 1
                target_call = 1 if phase == "first" else 2
            ...
            try:
                ...
                concept_builder.build_selected_concepts = original_builder
            assert observed["code"] == expected, (phase, mutation)


class _expect_phase2_error:
    def __enter__(self) -> dict[str, str | None]:
        self.observed: dict[str, str | None] = {"code": None}
        return self.observed

    def __exit__(self, exc_type, exc, traceback) -> bool:
        if exc_type is None:
            return False
        if not isinstance(exc, audit.Phase2LoadError):
            return False
        self.observed["code"] = exc.code
        return True


def test_phase2a_batch03_task42_rewrite_retains_locked_facts_and_strict_pattern_cards() -> None:
    root, _, context = _production_phase2a_batch03()
    baseline = context.baseline
    evidence = context.evidence
    assert isinstance(baseline, dict)
    ...
    assert len(locked_ids) == len(evidence_facts) == 94
    assert [fact["id"] for fact in evidence_facts] == locked_ids
    ...
    assert {
        ...
        if fact["disposition"] == "research-needed"
    } == unresolved_ids
    assert all(
        fact["coverage"] is None
        for fact in evidence_facts
        if fact["id"] in unresolved_ids
    )
    assert all(
        fact["disposition"] == "covered"
        and isinstance(fact["coverage"], list)
        and fact["coverage"]
        for fact in evidence_facts
        if fact["id"] not in unresolved_ids
    )

    for entry in evidence["notes"]:
        note = context.note_records[entry["slug"]]
        assert audit.validate_summary(note) == []
        bullet_lines = audit._summary_bullet_lines(note)
        assert bullet_lines
        assert all(audit.VALID_BULLET_RE.match(line) for line in bullet_lines)
        assert all(audit.FOOTNOTE_REFERENCE_RE.search(line) for line in bullet_lines)
        assert entry["summaryBulletEvidence"] == audit._generated_keypoints(note)
        assert entry["newUnsupportedFacts"] == 0

    assert evidence["manualReviewFactIds"] == sorted(unresolved_ids)
    assert evidence["status"] == "needs-review"
    assert (
        root
        / "docs/reports/nr-summary-rewrite/phase2a/generated/batch-03-pattern.json"
    ).is_file()


def test_phase2a_batch03_task42_exact_empty_projection_migration_is_source_backed() -> None:
    root, _, context = _production_phase2a_batch03()
    baseline = context.baseline
    evidence = context.evidence
    assert isinstance(baseline, dict)
    ...
    entry = next(
        ...
        if item["slug"] == "cerebrovascular-malformations"
    )
    note = context.note_records["cerebrovascular-malformations"]

    assert locked["summaryHeadings"] == []
    assert locked["originalSummary"] == ""
    assert locked["factUnits"] == []
    assert len(note.summaries) == 1
    assert [section.heading for section in note.summaries] == ["Summary"]
    assert len(audit._summary_bullet_lines(note)) == 3
    assert list(entry["sourceDefinitions"]) == ["1", "2", "3"]
    assert entry["facts"] == []
    assert entry["status"] == "verified"
    assert entry["sourceStatus"] == "existing-sufficient"
    assert entry["validation"]["factCoverage"] == {
        "total": 0,
        "covered": 0,
        "researchNeeded": 0,
        "manualReview": 0,
    ...
    current_bytes = (root / note.path).read_bytes()
    inserted_summary = note.original_summary.encode("utf-8")
    assert current_bytes.count(inserted_summary) == 1
    reconstructed = current_bytes.replace(inserted_summary, b"", 1)
    assert hashlib.sha256(reconstructed).hexdigest() == locked["originalSha256"]
    current_text = current_bytes.decode("utf-8")
    assert current_text.count("## Summary（有無AV分流之分類）") == 1
    assert current_text.count("## Summary（硬腦膜動靜脈瘻，Dural AV Fistula）") == 1
    assert current_text.count("## Summary（軟膜動靜脈瘻，Pial AV Fistula）") == 1


def test_phase2a_batch03_task42_exceptions_are_narrow_and_inventory_authenticated() -> None:
    root, _, context = _production_phase2a_batch03()
    assert isinstance(context.baseline, dict)
    locked = next(
        entry
        for entry in context.baseline["notes"]
        if entry["slug"] == "cerebrovascular-malformations"
    )
    note = context.note_records["cerebrovascular-malformations"]
    inventory = json.loads(
    ...
    assert audit._phase2_is_approved_empty_projection_migration(context, locked)
    assert not audit._phase2_is_approved_empty_projection_migration(
        replace(context, batch={**context.batch, "id": "batch-02-disease"}),
        locked,
    )
    assert not audit._phase2_is_approved_empty_projection_migration(
        context, {**locked, "slug": "forged-other-slug"}
    )
    assert not audit._phase2_is_approved_empty_projection_migration(
        context, {**locked, "originalSummary": "forged"}
    )
    assert audit._phase2_source_ref_overrides_are_batch_scoped(
        context, {"fact": ["1"]}
    )
    assert not audit._phase2_source_ref_overrides_are_batch_scoped(
        replace(context, batch={**context.batch, "id": "batch-02-disease"}),
        {"fact": ["1"]},
    )

    assert audit._inventory_allows_approved_empty_projection_migration(
        inventory_entry,
        note,
        repo_root=root,
    )
    assert not audit._inventory_allows_approved_empty_projection_migration(
        {**inventory_entry, "slug": "forged-other-slug"},
        note,
        repo_root=root,
    )
    assert not audit._inventory_allows_approved_empty_projection_migration(
        inventory_entry,
        replace(note, sha256="0" * 64),
        repo_root=root,
    )
    extra_heading_note = audit.parse_note_text(
        note.path,
        (
            (root / note.path).read_text(encoding="utf-8")
            + "\n## Summary\n- **Forged**: extra accepted heading.[^1]\n"
        ),
    )
    assert not audit._inventory_allows_approved_empty_projection_migration(
        inventory_entry,
    ...
    original_validator = audit.validate_phase2_batch
    audit.validate_phase2_batch = lambda *_args, **_kwargs: []
    try:
        assert audit._inventory_allows_approved_empty_projection_migration(
            inventory_entry,
            note,
        ...
        audit.validate_phase2_batch = original_validator


def test_phase2a_batch03_task42_evidence_regenerates_and_rejects_empty_projection_forgery() -> None:
    _, _, context = _production_phase2a_batch03()
    evidence = context.evidence
    assert isinstance(evidence, dict)
    queue = [
    ...
    regenerated = audit.build_phase2_rewrite_evidence(
        ...
        research_needed_fact_ids=queue,
        coverage_anchors_by_fact_id=_phase2_coverage_anchors(evidence),
    ...
    empty_entry["facts"] = [
        {
            "id": "cerebrovascular-malformations-f01",
            "sourceRefs": ["1"],
            "disposition": "covered",
            "coverage": [{"bulletIndex": 1, "quote": "無AV分流"}],
        }
    ]
    empty_entry["coverageEvidenceSha256"] = audit.phase2_coverage_evidence_sha256(
        canonical_sha256(context.baseline), empty_entry
    )
    codes = {
        finding.code
        for finding in audit.validate_phase2_batch(
    ...
    assert "evidence-fact-coverage" in codes

    mismatched = deepcopy(evidence)
    empty_entry = next(
        entry
        for entry in mismatched["notes"]
        if entry["slug"] == "cerebrovascular-malformations"
    )
    empty_entry["sourceDefinitions"]["1"]["citation"] += " forged"
    empty_entry["coverageEvidenceSha256"] = audit.phase2_coverage_evidence_sha256(
        canonical_sha256(context.baseline), empty_entry
    )
    codes = {
        finding.code
        for finding in audit.validate_phase2_batch(
            replace(context, evidence=mismatched),
            check_source_hashes=False,
            check_generated=False,
        )
    }
...
def test_phase2a_batch03_task43_approval_does_not_claim_tranche_acceptance() -> None:
    _, _, context = _production_phase2a_batch03()
    evidence = context.evidence
    assert isinstance(evidence, dict)
    assert evidence["workflow"] == {
    ...
    assert "phase2aVerification" not in evidence
    assert "generatedObservation" not in evidence


def _copy_production_phase2a_task43_checkout(destination: Path) -> Path:
    """Copy the complete reviewed predecessor chain and current Batch 03."""
    assignment_path = _copy_production_phase2a_batch02_checkout(destination)
    root, _, batch03 = _production_phase2a_batch03()
    relative_files = [
        batch03.baseline_path,
        batch03.evidence_path,
        *(Path(record.path) for record in batch03.note_records.values()),
...
def test_phase2a_batch03_task43_review_and_generated_seal_are_genuine() -> None:
    root, assignment_path, context = _production_phase2a_batch03()
    evidence = context.evidence
    assert isinstance(evidence, dict)
    baseline_digest = canonical_sha256(context.baseline)
    ...
    facts = [fact for entry in evidence["notes"] for fact in entry["facts"]]
    covered = [fact for fact in facts if fact["disposition"] == "covered"]
    unresolved = [fact for fact in facts if fact["disposition"] != "covered"]
    assert len(facts) == 94
    assert len(covered) == 91
    assert all(
        isinstance(fact["coverage"], list) and fact["coverage"]
        for fact in covered
    )
    assert unresolved == [
        {
            "id": "cns-opportunistic-infection-f03",
            "sourceRefs": [],
            "disposition": "research-needed",
            "coverage": None,
        },
        {
            "id": "cns-opportunistic-infection-f06",
            "sourceRefs": [],
            "disposition": "research-needed",
            "coverage": None,
        },
        {
            "id": "cns-opportunistic-infection-f07",
            "sourceRefs": [],
            "disposition": "research-needed",
            "coverage": None,
        },
    ...
    assert audit.TRUSTED_PHASE2A_GENERATED_OBSERVATION_SHA256 == {
        ...
        "batch-03-pattern": observation_digest,
    }
    assert checked["selectedSlugs"] == list(BATCH03_PATTERN_SLUGS)
    assert len(checked["detailFiles"]) == 10
    assert checked["detailFileCount"] == 980
    assert checked["index"]["entryCount"] == 980
    assert checked["nonselectedBefore"] == checked["nonselectedAfter"]
    assert checked["secondRun"] == {
        "changedPaths": [],
        "mtimeChangedPaths": [],
    }
    for batch_id in audit.ACTIVE_PHASE2A_BATCHES:
        batch_context = audit.load_phase2_batch(
            root, assignment_path, batch_id
        )
        assert audit.validate_phase2_batch(
            batch_context,
...
def test_phase2a_batch03_task43_workflow_gate_rejects_review_and_queue_forgery() -> None:
    cases = (
        ("unapproved", "phase2-review-sequence"),
        ("same-identity", "phase2-reviewer-conflict"),
        ("wrong-baseline", "phase2-review-sequence"),
        ("forged-queue", "phase2-manual-queue-mismatch"),
    )
    for mutation, expected_code in cases:
        with tempfile.TemporaryDirectory() as directory:
            shadow = Path(directory) / mutation
            assignment_path = _copy_production_phase2a_task43_checkout(shadow)
            evidence_path = (
                shadow
                / "docs/reports/nr-summary-rewrite/phase2a/evidence/"
                "batch-03-pattern.json"
...
def test_phase2a_batch03_task43_coverage_and_source_attacks_fail_closed() -> None:
    _, _, context = _production_phase2a_batch03()
    ...
    for mutation in (
        ...
        entry = next(
            item
            for item in forged["notes"]
            if item["slug"] == "brain-tumor-imaging"
        )
        fact = entry["facts"][0]
        if mutation == "deleted":
            fact["coverage"] = []
        elif mutation == "label-only":
            fact["coverage"] = [
                {
                    "bulletIndex": 1,
                    "quote": "**部位與年齡｜Pilocytic astrocytoma**",
                }
            ]
        elif mutation == "short-fragment":
            fact["coverage"] = [
                {"bulletIndex": 1, "quote": "好發小腦"}
            ]
        elif mutation == "duplicate":
            fact["coverage"] = [*fact["coverage"], *fact["coverage"]]
        elif mutation == "source-incomplete":
            entry["sourceDefinitions"].pop("1")
        else:
            entry = next(
                item
                for item in forged["notes"]
                if item["slug"] == "cns-opportunistic-infection"
            )
            fact = next(
                item
                for item in entry["facts"]
                if item["id"] == "cns-opportunistic-infection-f03"
            )
            fact["coverage"] = [
                {
                    "bulletIndex": 1,
                    "quote": "化膿性膿瘍中心膿液擴散明顯受限",
                }
            ]
        entry["coverageEvidenceSha256"] = (
            audit.phase2_coverage_evidence_sha256(
        ...
        expected = (
            ...
            if mutation == "source-incomplete"
            else "evidence-fact-coverage"
        )
        assert expected in codes, mutation


def test_phase2a_batch03_task43_registry_relocation_and_attacks_fail_closed() -> None:
    root, assignment_path, context = _production_phase2a_batch03()
    original_registry = audit.TRUSTED_PHASE2A_GENERATED_OBSERVATION_SHA256
    valid01 = original_registry.get("batch-01-anatomy")
    valid02 = original_registry.get("batch-02-disease")
    valid03 = original_registry.get("batch-03-pattern")
    registries = (
        {},
        {"batch-01-anatomy": valid01},
        {
        ...
        {
            "batch-01-anatomy": valid01,
            "batch-03-pattern": valid03,
        },
        {
            "batch-01-anatomy": valid01,
            "batch-02-disease": valid02,
            "batch-03-pattern": valid03,
            "batch-04-forged": "f" * 64,
        },
    )
    try:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        shadow_assignment = _copy_production_phase2a_task43_checkout(shadow)
        shadow_context = audit.load_phase2_batch(
            shadow, shadow_assignment, "batch-03-pattern"
        )
        assert audit.validate_phase2_batch(
            shadow_context,
            check_source_hashes=False,
        ...
        slug = "brain-tumor-imaging"
        detail_path = shadow / shadow_context.generated_root / f"{slug}.json"
        detail = json.loads(detail_path.read_text(encoding="utf-8"))
        detail["keyPoints"] = ["forged"]
        detail_path.write_text(
            json.dumps(detail, ensure_ascii=False, indent=2) + "\n",
...
def test_phase2a_batch03_task43_manifest_rejects_second_run_and_coordinated_reseal() -> None:
    with tempfile.TemporaryDirectory() as directory:
        shadow = Path(directory) / "manifest-attacks"
        assignment_path = _copy_production_phase2a_task43_checkout(shadow)
        manifest_path = (
            shadow
            / "docs/reports/nr-summary-rewrite/phase2a/generated/"
        ...
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest["secondRun"]["mtimeChangedPaths"] = [
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        detail_path = shadow / context.generated_root / f"{slug}.json"
        detail = json.loads(detail_path.read_text(encoding="utf-8"))
        detail["keyPoints"] = [*detail["keyPoints"], "forged"]
        detail_path.write_text(
            json.dumps(detail, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )

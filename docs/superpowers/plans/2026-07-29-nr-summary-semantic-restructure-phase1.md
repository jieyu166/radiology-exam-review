...
A valid top-level bullet matches the bold-label contract and contains at least one defined `[^id]` reference. Every
nonblank line in an accepted Summary variant must be either a nonempty `###` heading or a valid top-level bullet.
Plain prose, block quotes, fenced code, tables, callouts, and nested bullets are rejected. A note may contain multiple
accepted Summary variants, and every variant must contain at least one valid bullet.
...
- [ ] **Step 1: Run the batch-scoped deterministic build**
...
The builder selects the checked-in batch slugs, writes only selected detail files whose deterministic bytes differ,
and reconstructs a coherent index from checked-in detail files.

- [ ] **Step 2: Verify generated JSON scope, manifest, and idempotence**
...
Expected: only selected pilot JSON files whose bytes changed, plus `data/concepts-index.json` only when its coherent
content changed. The second build writes zero files and is byte-identical. Record a generated manifest containing the
exact 10 pilot hashes, index hash and entry count, total detail-file count, and a canonical whole-detail-tree digest.
Any missing, added, or changed non-pilot detail file fails the gate; never discard pre-existing user changes.
...
- [ ] **Step 4: Verify website keyPoints aggregate every accepted Summary variant**

Parse these exact files and compare `keyPoints` to the aggregate of top-level bullets from all accepted source Summary
variants, in source order, after footnote removal:
...
The current pilot root remains `needs-review` while its four derived manual fact units remain unresolved, and Phase 2
stays disabled. Independently verify inventory at exactly `216 / 10 / 206`. Record command results, exact lint
`2 errors / 124 warnings`, generated manifest, manual queue, and changed file lists in `batch-00.json`. Seal the final
evidence with a trusted anchor covering fact dispositions and mappings, per-note sourceStatus/status, current and
rewritten Summary snapshots, validation, root status, and all Phase 1 verification metadata. A coordinated evidence
mutation without trusted resealing must fail with `evidence-trusted-final-mismatch`.
